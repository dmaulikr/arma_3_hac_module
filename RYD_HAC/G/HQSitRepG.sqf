...
while {not ((isNull RydHQG) or (RydHQG_Surrender))} do
	...
	if not (isNil ("RydHQG_CivF")) then {_civF = RydHQG_CivF};
		...
		if not ((isNull (leaderHQG)) and not (isNull _x) and (alive (leaderHQG)) and (alive (leader _x)) and not (_isCaptive)) then
			...
			if ((RydHQG_Front) and ((side _x) getFriend (side RydHQG) < 0.6) and ((getPosATL (vehicle (leader _x))) in FrontA) and not (_isCiv)) then {if not (_x in RydHQG_Enemies) then {RydHQG_Enemies set [(count RydHQG_Enemies), _x]}};
	...
	if (RydHQG_Init) then {[RydHQG] spawn G_Desperado};
	...
	if (RydHQG_Flee) then
		...
		if not ((count RydHQG_Friends) == 0) then {_midCow = _AllCow/(count RydHQG_Friends)};
			...
			if (((random ((20 + (RydHQG_Morale/5))/_AllPanic)) < _cowF) and ((random 100) > (100/(_AllPanic + 1)))) then 
				...
				if (RydHQG_DebugII) then {_i = [(getPosATL (vehicle (leader _x))),_x,"markPanic","ColorYellow","ICON","mil_dot","A!","A!",[0.5,0.5]] call RYD_Mark};
				...
				if (RydHQG_Surr) then
					...
					if ((random 100) > 50) then
						{
						if (RydHQG_DebugII) then {_i setMarkerColor "ColorPink";_i setMarkerText "A!!!"};
						_isCaptive = _x getVariable ("isCaptive" + (str _x));
						if (isNil "_isCaptive") then {_isCaptive = false};
