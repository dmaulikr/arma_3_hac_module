...
while {not ((isNull RydHQE) or (RydHQE_Surrender))} do
	...
	if not (isNil ("RydHQE_CivF")) then {_civF = RydHQE_CivF};
		...
		if not ((isNull (leaderHQE)) and not (isNull _x) and (alive (leaderHQE)) and (alive (leader _x)) and not (_isCaptive)) then
			...
			if ((RydHQE_Front) and ((side _x) getFriend (side RydHQE) < 0.6) and ((getPosATL (vehicle (leader _x))) in FrontA) and not (_isCiv)) then {if not (_x in RydHQE_Enemies) then {RydHQE_Enemies set [(count RydHQE_Enemies), _x]}};
	...
	if (RydHQE_Init) then {[RydHQE] spawn E_Desperado};
	...
	if (RydHQE_Flee) then
		...
		if not ((count RydHQE_Friends) == 0) then {_midCow = _AllCow/(count RydHQE_Friends)};
			...
			if (((random ((20 + (RydHQE_Morale/5))/_AllPanic)) < _cowF) and ((random 100) > (100/(_AllPanic + 1)))) then 
				...
				if (RydHQE_DebugII) then {_i = [(getPosATL (vehicle (leader _x))),_x,"markPanic","ColorYellow","ICON","mil_dot","A!","A!",[0.5,0.5]] call RYD_Mark};
				...
				if (RydHQE_Surr) then
					...
					if ((random 100) > 50) then
						{
						if (RydHQE_DebugII) then {_i setMarkerColor "ColorPink";_i setMarkerText "A!!!"};
						_isCaptive = _x getVariable ("isCaptive" + (str _x));
						if (isNil "_isCaptive") then {_isCaptive = false};
	...
	if (((count RydHQE_KnEnemies) > 0) and ((count RydHQE_ArtG) > 0)) then {[RydHQE_ArtG,RydHQE_KnEnemies,(RydHQE_EnHArmor + RydHQE_EnMArmor + RydHQE_EnLArmor),RydHQE_Friends,RydHQE_Debug,leaderHQE] call RYD_CFF};
