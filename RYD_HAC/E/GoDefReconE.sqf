...
if (_ammo == 0) exitwith {RydHQE_RecDefSpot = RydHQE_RecDefSpot - [_unitG];RydHQE_GoodSpots = RydHQE_GoodSpots + [_DefPos];RydHQE_Roger = true};
...
if ((_busy) and (_unitG in RydHQE_RecDefSpot)) exitwith {RydHQE_Roger = true};
...
if (_isWater) exitwith {RydHQE_RecDefSpot = RydHQE_RecDefSpot - [_unitG];RydHQE_Roger = true;_unitG setVariable [("Busy" + (str _unitG)),false]};
...
if not (_alive) exitwith {if ((RydHQE_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markDef" + str (_unitG))};RydHQE_RecDefSpot = RydHQE_RecDefSpot - [_unitG]};
...
if ((RydHQE_Debug) or (isPlayer (leader _unitG))) then 
	{
	_i = [_TED,_unitG,"markWatch","ColorGreen","ICON","mil_dot","E","E",[0.2,0.2]] call RYD_Mark
	};
