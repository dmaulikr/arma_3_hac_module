...
while {not ((isNull RydHQH) or (RydHQH_Surrender))} do
	...
	if not (isNil ("RydHQH_CivF")) then {_civF = RydHQH_CivF};
		...
		if not ((isNull (leaderHQH)) and not (isNull _x) and (alive (leaderHQH)) and (alive (leader _x)) and not (_isCaptive)) then
			...
			if ((RydHQH_Front) and ((side _x) getFriend (side RydHQH) < 0.6) and ((getPosATL (vehicle (leader _x))) in FrontA) and not (_isCiv)) then {if not (_x in RydHQH_Enemies) then {RydHQH_Enemies set [(count RydHQH_Enemies), _x]}};
	...
	if (RydHQH_Init) then {[RydHQH] spawn H_Desperado};
	...
	if (RydHQH_Flee) then
		...
		if not ((count RydHQH_Friends) == 0) then {_midCow = _AllCow/(count RydHQH_Friends)};
			...
			if (((random ((20 + (RydHQH_Morale/5))/_AllPanic)) < _cowF) and ((random 100) > (100/(_AllPanic + 1)))) then 
				...
				if (RydHQH_DebugII) then {_i = [(getPosATL (vehicle (leader _x))),_x,"markPanic","ColorYellow","ICON","mil_dot","A!","A!",[0.5,0.5]] call RYD_Mark};
				...
				if (RydHQH_Surr) then
					...
					if ((random 100) > 50) then
						{
						if (RydHQH_DebugII) then {_i setMarkerColor "ColorPink";_i setMarkerText "A!!!"};
						_isCaptive = _x getVariable ("isCaptive" + (str _x));
						if (isNil "_isCaptive") then {_isCaptive = false};
	...
	if (((count RydHQH_KnEnemies) > 0) and ((count RydHQH_ArtG) > 0)) then {[RydHQH_ArtG,RydHQH_KnEnemies,(RydHQH_EnHArmor + RydHQH_EnMArmor + RydHQH_EnLArmor),RydHQH_Friends,RydHQH_Debug,leaderHQH] call RYD_CFF};
