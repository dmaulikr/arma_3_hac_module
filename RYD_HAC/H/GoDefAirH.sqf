...
if (_ammo == 0) exitwith {RydHQH_Roger = true};
...
if ((_busy) and (_unitG in RydHQH_AirInDef)) exitwith {RydHQH_Roger = true};
...
RydHQH_AirInDef = RydHQH_AirInDef + [_unitG];
...
while {not (_endThis)} do
	...
	if ((RydHQH_Debug) or (isPlayer (leader _unitG))) then 
		{
		_i = [_DefPos,_unitG,"markDef","ColorBrown","ICON","mil_dot","Air H"," - DEFEND AREA"] call RYD_Mark
		};
	...
	if (_unitG in RydHQH_BAirG) then 
		{
		_chosen = GrpNull;
		_dstM = 5000;

		_chosen = ([_Spot,RydHQH_KnEnemiesG,0] call RYD_CloseEnemyB) select 2;
...
if not (_alive) exitwith {if ((RydHQH_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markDef" + (str _unitG))};RydHQH_AirInDef = RydHQH_AirInDef - [_unitG]};

if ((isPlayer (leader _unitG)) and not (isMultiplayer)) then {(leader _unitG) removeSimpleTask _task};

sleep 30;

if ((RydHQH_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markDef" + (str _unitG))};
