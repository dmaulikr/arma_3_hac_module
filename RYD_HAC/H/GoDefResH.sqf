...
_DefPos = _this select 1;if (_unitG in RydHQH_Garrison) exitwith {RydHQH_DefSpot = RydHQH_DefSpot - [_unitG];RydHQH_GoodSpots = RydHQH_GoodSpots + [_DefPos];RydHQH_Roger = true};
...
if ((_unitG in RydHQH_NCCargoG) and ((count (units _unitG)) <= 1)) then 
	{
	_PosX = ((position leaderHQH) select 0) + (random 200) - 100;
	_PosY = ((position leaderHQH) select 1) + (random 200) - 100;
	_DefPos = [_posX,_posY]
	};
...
if ((RydHQH_Debug) or (isPlayer (leader _unitG))) then 
	{
	_i = [_DefPos,_unitG,"markDef","ColorRed","ICON","mil_dot","DRes H"," - DEFEND POSITION"] call RYD_Mark
	};
...
_dX = 2000 * (sin RydHQH_Angle);
_dY = 2000 * (cos RydHQH_Angle);
...
if ((RydHQH_Debug) or (isPlayer (leader _unitG))) then 
	{
	_i = [_TED,_unitG,"markWatch","ColorGreen","ICON","mil_dot","H","H",[0.2,0.2]] call RYD_Mark
	};
...
waituntil
	...
	if (_sumD > 0) then {_dstAv = _sumD/(count RydHQH_Friends)};
	...
	if ((count _tooClose) > 0) then 
		{
		_HQinDanger = true;
		_HQsupporting = RydHQH getVariable "SPRTD";
		if (isNil "_HQsupporting") then {_HQsupporting = 0}
		};

	_HQchance = (80 - (_HQsupporting * 20))/(1 + (RydHQH_Recklessness * 0.5));
	...
	if ((_HQinDanger) and ((random 100) < _HQchance)) then
		...
		_unitG setVariable ["Reinforcing",RydHQH];
		RydHQH setVariable ["SPRTD",_HQsupporting + 1]
		}
	else
		{
		_ToReinf = RydHQH_Friends - (RydHQH_AirG + RydHQH_DefRes);
	...
	if not (isNull _chosen) then 
		...
		if ((RydHQH_Debug) or (isPlayer (leader _unitG))) then 
			{
			_i = [_RnfP,_unitG,"markReinf","ColorRed","ICON","mil_dot","Reinf H"," - REINFORCE POSITION",[0.3,0.3]] call RYD_Mark
			}
		};

	
	if not (_unitG getVariable "Defending") then {_endThis = true};
	if ((isNull _unitG) or (isNull RydHQH)) then {_endThis = true;_alive = false};
...
if ((RydHQH_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markDef" + (str _unitG));deleteMarker ("markWatch" + (str _unitG));deleteMarker ("markReinf" + (str _unitG))};
