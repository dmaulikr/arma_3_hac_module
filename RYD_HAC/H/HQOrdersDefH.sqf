		...
		while {((count _airDef) > (count RydHQH_AirInDef))} do
			...
			if not (_isDef) then
				...
				if not (_x in RydHQH_Def) then 
					...
					switch (true) do 
						{
						case (_x in RydHQH_InfantryG) : {_expression = "(1 + (2 * Houses)) * (1 + (1.5 * Forest)) * (1 + Trees) * (1 - Meadow) * (1 - (10 * sea))"};
						case (not (_x in RydHQH_InfantryG)) : {_expression = "(1 + (2 * Meadow)) * (1 - Forest) * (1 - (0.5 * Trees)) * (1 - (10 * sea))"};
						};
