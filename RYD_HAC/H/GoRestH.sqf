...
if not (isNull _AV) then
	...
	if not (_GDV == _unitG) then
		{
		if not (_GDV in RydHQH_Exhausted) then
			{
	...
	else
		{
			...
			if not ((group _x) == _unitG) then 
				{
				if not ((group _x) in RydHQH_Exhausted) then
					{
					[_x] ordergetIn false;
					unassignVehicle _x
					}
...
if (_unitG in RydHQH_Garrison) exitwith {RydHQH_Exhausted = RydHQH_Exhausted - [_unitG]};
...
_Xpos = ((position leaderHQH) select 0) + (random 500) - 250;
_Ypos = ((position leaderHQH) select 1) + (random 500) - 250;
...
if not (isNil "RydHQH_RestDecoy") then
	{
	_isDecoy = true;

	_tRadius = (triggerArea RydHQH_RestDecoy) select 0;

	if ((random 100) >= RydHQH_RDChance) exitWith {_isDecoy = false};

	_tPos = position RydHQH_RestDecoy;
	_enemyMatters = (triggerArea RydHQH_RestDecoy) select 3;
...
waituntil
	...
	(not (_isWater) and ((isNull (leaderHQH FindNearestEnemy [_posX,_posY])) or (((leaderHQH FindNearestEnemy [_posX,_posY]) distance [_posX,_posY]) >= 500) or not (_enemyMatters)) or (_counter > 30))
	};

if ((_counter > 30) or (not (isNull (leaderHQH FindNearestEnemy [_posX,_posY])) and (((leaderHQH FindNearestEnemy [_posX,_posY]) distance [_posX,_posY]) < 500) and (_enemyMatters))) then {_posX = ((position leaderHQH) select 0) + (random 500) - 250;_posY = ((position leaderHQH) select 1) + (random 500) - 250};

_isWater = surfaceIsWater [_posX,_posY];
if ((_isWater) or (not (isNull (leaderHQH FindNearestEnemy [_posX,_posY])) and (((leaderHQH FindNearestEnemy [_posX,_posY]) distance [_posX,_posY]) < 500) and (_enemyMatters))) exitwith {_unitG setVariable [("Resting" + (str _unitG)),false,true];_unitG setVariable [("Busy" + (str _unitG)), false, true];RydHQH_Exhausted = RydHQH_Exhausted - [_unitG]};
...
if ((RydHQH_Debug) or (isPlayer (leader _unitG))) then
	{
	_i = [[_posX,_posY],_unitG,"markRest","ColorGreen","ICON","mil_triangle","H"," - REST & REGROUP",[0.5,0.5]] call RYD_Mark
	};
...
if not (_alive) exitwith {if ((RydHQH_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markRest" + str (_unitG))};RydHQH_Exhausted = RydHQH_Exhausted - [_unitG]};
...
waituntil 
	...
	if (((_Gdamage/(_current + 0.1)) > (0.4*((RydHQH_Recklessness/1.2) + 1))) or not (_effective) or not (_ammo)) then {_solready = false};
		...
		if (not (isNull _veh) and (not (canMove _veh) or ((fuel _veh) <= 0.1) or ((damage _veh) > 0.5) or (((group _x) in ((RydHQH_AirG - RydHQH_NCAirG) + (RydHQH_HArmorG + RydHQH_LArmorG + (RydHQH_CarsG - (RydHQH_NCCargoG + RydHQH_SupportG))))) and ((count (magazines _veh)) == 0)) and not ((group _x) in RydHQH_RAirG))) exitwith {_vehready = false};
		}
	foreach (units _unitG);
...
if ((RydHQH_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markRest" + str (_unitG))};
RydHQH_Exhausted = RydHQH_Exhausted - [_unitG];
