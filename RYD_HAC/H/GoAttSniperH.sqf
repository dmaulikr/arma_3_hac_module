...
_dX = (_PosObj1 select 0) - ((getPosATL leaderHQH) select 0);
_dY = (_PosObj1 select 1) - ((getPosATL leaderHQH) select 1);
...
if (_unitG in RydHQH_AirG) then {_distance2 = 800};
...
_posX = ((getPosATL leaderHQH) select 0) + _dXb + (random 200) - 100;
_posY = ((getPosATL leaderHQH) select 1) + _dYb + (random 200) - 100;
...
if ((RydHQH_Debug) or (isPlayer (leader _unitG))) then 
	{
	_i = [[_posX,_posY],_unitG,"markAttack","ColorRed","ICON","mil_dot","Snp H"," - ATTACK"] call RYD_Mark
	};
...
if not (_alive) exitwith {[_Trg,"SnpAttacked"] call RYD_VarReductor;if ((RydHQH_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markAttack" + str (_unitG))}};
