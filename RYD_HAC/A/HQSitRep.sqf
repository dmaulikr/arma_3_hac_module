...
while {not ((isNull RydHQ) or (RydHQ_Surrender))} do
	...
	if not (isNil ("RydHQ_CivF")) then {_civF = RydHQ_CivF};
		...
		if not ((isNull (leaderHQ)) and not (isNull _x) and (alive (leaderHQ)) and (alive (leader _x)) and not (_isCaptive)) then
			...
			if ((RydHQ_Front) and ((side _x) getFriend (side RydHQ) < 0.6) and ((getPosATL (vehicle (leader _x))) in FrontA) and not (_isCiv)) then {if not (_x in RydHQ_Enemies) then {RydHQ_Enemies set [(count RydHQ_Enemies), _x]}};
	...
	if (RydHQ_Init) then {[RydHQ] spawn A_Desperado};
	...
	if (RydHQ_Flee) then
		...
		if not ((count RydHQ_Friends) == 0) then {_midCow = _AllCow/(count RydHQ_Friends)};
			...
			if (((random ((20 + (RydHQ_Morale/5))/_AllPanic)) < _cowF) and ((random 100) > (100/(_AllPanic + 1)))) then 
				...
				if (RydHQ_DebugII) then {_i = [(getPosATL (vehicle (leader _x))),_x,"markPanic","ColorYellow","ICON","mil_dot","A!","A!",[0.5,0.5]] call RYD_Mark};
	...
	if (((count RydHQ_KnEnemies) > 0) and ((count RydHQ_ArtG) > 0)) then {[RydHQ_ArtG,RydHQ_KnEnemies,(RydHQ_EnHArmor + RydHQ_EnMArmor + RydHQ_EnLArmor),RydHQ_Friends,RydHQ_Debug,leaderHQ] call RYD_CFF};
