...
if (_unitG in RydHQ_Garrison) exitwith {};
...
if (isNil ("RydHQ_Obj")) then {_default = position leaderHQ} else {_default = position RydHQ_Obj};
...
if not (isNil ("RydHQ_Obj")) then 
	{
	_max0Enemy = RydHQ_Obj;
	_min0Enemy = RydHQ_Obj;

	_max1Enemy = RydHQ_Obj;
	_min1Enemy = RydHQ_Obj
	};

if not ((count RydHQ_KnEnemies) == 0) then 
	{
	_max0Enemy = RydHQ_KnEnemies select _sel0Max;
	_min0Enemy = RydHQ_KnEnemies select _sel0Min;

	_max1Enemy = RydHQ_KnEnemies select _sel1Max;
	_min1Enemy = RydHQ_KnEnemies select _sel1Min
	};

_PosMid0 = (_Epos0Min + _Epos0Max)/2;
_PosMid1 = (_Epos1Min + _Epos1Max)/2;

_dX = (_PosMid0) - ((getPosATL leaderHQ) select 0);
_dY = (_Posmid1) - ((getPosATL leaderHQ) select 1);
...
if ((_ammo > 0) and not (_busy)) then
	...
	RydHQ_SFTargets set [(count RydHQ_SFTargets),_trgG];
