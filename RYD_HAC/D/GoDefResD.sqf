...
_DefPos = _this select 1;if (_unitG in RydHQD_Garrison) exitwith {RydHQD_DefSpot = RydHQD_DefSpot - [_unitG];RydHQD_GoodSpots = RydHQD_GoodSpots + [_DefPos];RydHQD_Roger = true};
...
if ((_unitG in RydHQD_NCCargoG) and ((count (units _unitG)) <= 1)) then 
	{
	_PosX = ((position leaderHQD) select 0) + (random 200) - 100;
	_PosY = ((position leaderHQD) select 1) + (random 200) - 100;
	_DefPos = [_posX,_posY]
	};
...
if ((RydHQD_Debug) or (isPlayer (leader _unitG))) then 
	{
	_i = [_DefPos,_unitG,"markDef","ColorRed","ICON","mil_dot","DRes D"," - DEFEND POSITION"] call RYD_Mark
	};
...
_dX = 2000 * (sin RydHQD_Angle);
_dY = 2000 * (cos RydHQD_Angle);
...
if ((RydHQD_Debug) or (isPlayer (leader _unitG))) then 
	{
	_i = [_TED,_unitG,"markWatch","ColorGreen","ICON","mil_dot","D","D",[0.2,0.2]] call RYD_Mark
	};
...
waituntil
	...
	if (_sumD > 0) then {_dstAv = _sumD/(count RydHQD_Friends)};
	...
	if ((count _tooClose) > 0) then 
		{
		_HQinDanger = true;
		_HQsupporting = RydHQD getVariable "SPRTD";
		if (isNil "_HQsupporting") then {_HQsupporting = 0}
		};

	_HQchance = (80 - (_HQsupporting * 20))/(1 + (RydHQD_Recklessness * 0.5));
	...
	if ((_HQinDanger) and ((random 100) < _HQchance)) then
		...
		_unitG setVariable ["Reinforcing",RydHQD];
		RydHQD setVariable ["SPRTD",_HQsupporting + 1]
		}
	else
		{
		_ToReinf = RydHQD_Friends - (RydHQD_AirG + RydHQD_DefRes);
	...
	if not (isNull _chosen) then 
		...
		if ((RydHQD_Debug) or (isPlayer (leader _unitG))) then 
			{
			_i = [_RnfP,_unitG,"markReinf","ColorRed","ICON","mil_dot","Reinf D"," - REINFORCE POSITION",[0.3,0.3]] call RYD_Mark
			}
		};

	
	if not (_unitG getVariable "Defending") then {_endThis = true};
	if ((isNull _unitG) or (isNull RydHQD)) then {_endThis = true;_alive = false};
...
if ((RydHQD_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markDef" + (str _unitG));deleteMarker ("markWatch" + (str _unitG));deleteMarker ("markReinf" + (str _unitG))};
