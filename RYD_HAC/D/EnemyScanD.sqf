...
if (RydHQD_DebugII) then
	{
	waitUntil 
		{
		sleep 1;
		not (isNil "RydHQD_KnEnemiesG")	
		};
...
while {not (isNull RydHQD)} do
	{
	_LCU = RydHQD_Friends - (RydHQD_NavalG + RydHQD_SupportG + RydHQD_ArtG + (RydHQD_NCCargoG - (RydHQD_NCrewInfG - RydHQD_SupportG)));
	...
	foreach (RydHQD_Friends - RydHQD_AirG);
