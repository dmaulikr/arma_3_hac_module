...
while {not ((isNull RydHQD) or (RydHQD_Surrender))} do
	...
	if not (isNil ("RydHQD_CivF")) then {_civF = RydHQD_CivF};
		...
		if not ((isNull (leaderHQD)) and not (isNull _x) and (alive (leaderHQD)) and (alive (leader _x)) and not (_isCaptive)) then
			...
			if ((RydHQD_Front) and ((side _x) getFriend (side RydHQD) < 0.6) and ((getPosATL (vehicle (leader _x))) in FrontA) and not (_isCiv)) then {if not (_x in RydHQD_Enemies) then {RydHQD_Enemies set [(count RydHQD_Enemies), _x]}};
	...
	if (RydHQD_Init) then {[RydHQD] spawn D_Desperado};
	...
	if (RydHQD_Flee) then
		...
		if not ((count RydHQD_Friends) == 0) then {_midCow = _AllCow/(count RydHQD_Friends)};
			...
			if (((random ((20 + (RydHQD_Morale/5))/_AllPanic)) < _cowF) and ((random 100) > (100/(_AllPanic + 1)))) then 
				...
				if (RydHQD_DebugII) then {_i = [(getPosATL (vehicle (leader _x))),_x,"markPanic","ColorYellow","ICON","mil_dot","A!","A!",[0.5,0.5]] call RYD_Mark};
				...
				if (RydHQD_Surr) then
					...
					if ((random 100) > 50) then
						{
						if (RydHQD_DebugII) then {_i setMarkerColor "ColorPink";_i setMarkerText "A!!!"};
						_isCaptive = _x getVariable ("isCaptive" + (str _x));
						if (isNil "_isCaptive") then {_isCaptive = false};
