...
_DefPos = _this select 1;if (_unitG in RydHQD_Garrison) exitwith {RydHQD_RecDefSpot = RydHQD_RecDefSpot - [_unitG];RydHQD_GoodSpots = RydHQD_GoodSpots + [_DefPos];RydHQD_Roger = true};
_angleV = _this select 2;


_ammo = [_unitG] call RYD_AmmoCount;

if (_ammo == 0) exitwith {RydHQD_RecDefSpot = RydHQD_RecDefSpot - [_unitG];RydHQD_GoodSpots = RydHQD_GoodSpots + [_DefPos];RydHQD_Roger = true};
...
if ((_busy) and (_unitG in RydHQD_RecDefSpot)) exitwith {RydHQD_Roger = true};
...
if (_isWater) exitwith {RydHQD_RecDefSpot = RydHQD_RecDefSpot - [_unitG];RydHQD_Roger = true;_unitG setVariable [("Busy" + (str _unitG)),false]};
...
if ((RydHQD_Debug) or (isPlayer (leader _unitG))) then 
	{
	_i = [_DefPos,_unitG,"markDef","ColorBrown","ICON","mil_dot","Rec D"," - WATCH FOREGROUND"] call RYD_Mark
	};
...
if not (_alive) exitwith {if ((RydHQD_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markDef" + str (_unitG))};RydHQD_RecDefSpot = RydHQD_RecDefSpot - [_unitG]};
...
if not (_alive) exitWith 
	...
	RydHQD_RecDefSpot = RydHQD_RecDefSpot - [_unitG]
	};
