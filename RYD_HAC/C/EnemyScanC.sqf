...
if (RydHQC_DebugII) then
	{
	waitUntil 
		{
		sleep 1;
		not (isNil "RydHQC_KnEnemiesG")	
		};
...
while {not (isNull RydHQC)} do
	{
	_LCU = RydHQC_Friends - (RydHQC_NavalG + RydHQC_SupportG + RydHQC_ArtG + (RydHQC_NCCargoG - (RydHQC_NCrewInfG - RydHQC_SupportG)));
	...
	foreach (RydHQC_Friends - RydHQC_AirG);
