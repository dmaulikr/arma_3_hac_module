...
while {not ((isNull RydHQC) or (RydHQC_Surrender))} do
	...
	if not (isNil ("RydHQC_CivF")) then {_civF = RydHQC_CivF};
		...
		if not ((isNull (leaderHQC)) and not (isNull _x) and (alive (leaderHQC)) and (alive (leader _x)) and not (_isCaptive)) then
			...
			if ((RydHQC_Front) and ((side _x) getFriend (side RydHQC) < 0.6) and ((getPosATL (vehicle (leader _x))) in FrontA) and not (_isCiv)) then {if not (_x in RydHQC_Enemies) then {RydHQC_Enemies set [(count RydHQC_Enemies), _x]}};
	...
	if (RydHQC_Init) then {[RydHQC] spawn C_Desperado};
	...
	if (RydHQC_Flee) then
		...
		if not ((count RydHQC_Friends) == 0) then {_midCow = _AllCow/(count RydHQC_Friends)};
			...
			if (((random ((20 + (RydHQC_Morale/5))/_AllPanic)) < _cowF) and ((random 100) > (100/(_AllPanic + 1)))) then 
				...
				if (RydHQC_DebugII) then {_i = [(getPosATL (vehicle (leader _x))),_x,"markPanic","ColorYellow","ICON","mil_dot","A!","A!",[0.5,0.5]] call RYD_Mark};
				...
				if (RydHQC_Surr) then
					...
					if ((random 100) > 50) then
						{
						if (RydHQC_DebugII) then {_i setMarkerColor "ColorPink";_i setMarkerText "A!!!"};
						_isCaptive = _x getVariable ("isCaptive" + (str _x));
						if (isNil "_isCaptive") then {_isCaptive = false};
	...
	if (((count RydHQC_KnEnemies) > 0) and ((count RydHQC_ArtG) > 0)) then {[RydHQC_ArtG,RydHQC_KnEnemies,(RydHQC_EnHArmor + RydHQC_EnMArmor + RydHQC_EnLArmor),RydHQC_Friends,RydHQC_Debug,leaderHQC] call RYD_CFF};
