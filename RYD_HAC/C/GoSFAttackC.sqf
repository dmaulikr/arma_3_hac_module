...
if (_unitG in RydHQC_Garrison) exitwith {};
...
if (isNil ("RydHQC_Obj")) then {_default = position leaderHQC} else {_default = position RydHQC_Obj};
...
_max0Enemy = leaderHQC;
_min0Enemy = leaderHQC;

_max1Enemy = leaderHQC;
_min1Enemy = leaderHQC;

if not (isNil ("RydHQC_Obj")) then 
	{
	_max0Enemy = RydHQC_Obj;
	_min0Enemy = RydHQC_Obj;

	_max1Enemy = RydHQC_Obj;
	_min1Enemy = RydHQC_Obj
	};

if not ((count RydHQC_KnEnemies) == 0) then 
	{
	_max0Enemy = RydHQC_KnEnemies select _sel0Max;
	_min0Enemy = RydHQC_KnEnemies select _sel0Min;

	_max1Enemy = RydHQC_KnEnemies select _sel1Max;
	_min1Enemy = RydHQC_KnEnemies select _sel1Min
	};

_PosMid0 = (_Epos0Min + _Epos0Max)/2;
_PosMid1 = (_Epos1Min + _Epos1Max)/2;

_dX = (_PosMid0) - ((getPosATL leaderHQC) select 0);
_dY = (_Posmid1) - ((getPosATL leaderHQC) select 1);
...
_dX = _BEposX - ((getPosATL leaderHQC) select 0);
_dY = _BEposY - ((getPosATL leaderHQC) select 1);
...
_BorHQD = leaderHQC distance _BEnemyPos;
...
_posXWP1 = ((getPosATL leaderHQC) select 0) + _FlankPosX + _safeX1 + (random 200) - 100;
_posYWP1 = ((getPosATL leaderHQC) select 1) + _FlankPosY + _safeY1 + (random 200) - 100;
...
if ((leaderHQC distance [_posXWP1,_posYWP1]) > (leaderHQC distance [_posXWP2,_posYWP2])) then 
	{
...
if ((_ammo > 0) and not (_busy)) then
	...
	RydHQC_SFTargets set [(count RydHQC_SFTargets),_trgG];
	...
	if ((RydHQC_Debug) or (isPlayer (leader _unitG))) then 
		{
		_i1 = [[_posXWP1,_posYWP1],_unitG,"markSFFlank1","ColorOrange","ICON","mil_dot","SF1 C"," - SF FLANK 1"] call RYD_Mark;
		_i2 = [[_posXWP2,_posYWP2],_unitG,"markSFFlank2","ColorOrange","ICON","mil_dot","SF2 C"," - SF FLANK 2"] call RYD_Mark;
		_i3 = [[_posXWP3,_posYWP3],_unitG,"markSFFlank3","ColorOrange","ICON","mil_dot","SF3 C"," - SF FLANK 3"] call RYD_Mark;
		_i4 = [[_posXWP4,_posYWP4],_unitG,"markSFFlank4","ColorOrange","ICON","mil_dot","SF4 C"," - SF ATTACK"] call RYD_Mark
		};

	_CargoCheck = _unitG getvariable ("CC" + _unitvar);
	if (isNil ("_CargoCheck")) then {_unitG setVariable [("CC" + _unitvar), false]};
	_AV = assignedVehicle _UL;
	if ((RydHQC_CargoFind > 0) and (isNull _AV) and (([_posXWP4,_posYWP4] distance (vehicle _UL)) > 1000)) then {[_unitG] spawn C_SCargo } else {_unitG setVariable [("CC" + _unitvar), true]};
	...
	if ((isNull (leader (_this select 0))) or (_timer > 900)) exitwith {if ((RydHQC_Debug) or (isPlayer (leader _unitG))) then {{deleteMarker _x} foreach [_i1,_i2,_i3,_i4]};if not (isNull _GDV) then {[_GDV, (currentWaypoint _GDV)] setWaypointPosition [getPosATL (vehicle (leader _GDV)), 1];_GDV setVariable [("Busy" + _unitvar), false];}};
	...
	_beh = "AWARE";
	if (not (isNull _AV) and (_GDV in RydHQC_AirG)) then {_beh = "CARELESS"};
	_TO = [0,0,0];
	...
	if not (((group _DAV) == (group _UL)) or (isNull (group _DAV))) then 
		...
		_cause = [_GDV,6,true,300,30,[RydHQC_AirG,RydHQC_KnEnemiesG],true] call RYD_Wait;
		_timer = _cause select 0;
		...
		{
		_cause = [_unitG,6,true,300,30,[RydHQC_AirG,RydHQC_KnEnemiesG],false] call RYD_Wait;
		_timer = _cause select 0;
		_alive = _cause select 1;
		_enemy = _cause select 2
		};
	...
	if (not (_alive) and not (_OtherGroup)) exitwith {if ((RydHQC_Debug) or (isPlayer (leader _unitG))) then {{deleteMarker _x} foreach [_i1,_i2,_i3,_i4]}};
	if (isNull (leader (_this select 0))) exitwith {if ((RydHQC_Debug) or (isPlayer (leader _unitG))) then {{deleteMarker _x} foreach [_i1,_i2,_i3,_i4]};if not (isNull _GDV) then {[_GDV, (currentWaypoint _GDV)] setWaypointPosition [getPosATL (vehicle (leader _GDV)), 1];_GDV setVariable [("Busy" + _unitvar), false];}};
	...
	if ((RydHQC_Debug) or (isPlayer (leader _unitG))) then {_i1 setMarkerColor "ColorBlue"};
	...
	if ((RydHQC_Debug) or (isPlayer (leader _unitG))) then {_i2 setMarkerColor "ColorBlue"};
	...
	if ((isNull (leader (_this select 0))) or (_timer > 240)) exitwith {if ((RydHQC_Debug) or (isPlayer (leader _unitG))) then {{deleteMarker _x} foreach [_i1,_i2,_i3,_i4]};if not (isNull _GDV) then {[_GDV, (currentWaypoint _GDV)] setWaypointPosition [getPosATL (vehicle (leader _GDV)), 1];_GDV setVariable [("Busy" + _unitvar), false];_pass orderGetIn true}};
	...
	if ((RydHQC_Debug) or (isPlayer (leader _unitG))) then {_i3 setMarkerColor "ColorBlue"};
