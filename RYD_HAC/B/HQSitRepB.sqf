...
while {not ((isNull RydHQB) or (RydHQB_Surrender))} do
	...
	if not (isNil ("RydHQB_CivF")) then {_civF = RydHQB_CivF};
		...
		if not ((isNull (leaderHQB)) and not (isNull _x) and (alive (leaderHQB)) and (alive (leader _x)) and not (_isCaptive)) then
			...
			if ((RydHQB_Front) and ((side _x) getFriend (side RydHQB) < 0.6) and ((getPosATL (vehicle (leader _x))) in FrontA) and not (_isCiv)) then {if not (_x in RydHQB_Enemies) then {RydHQB_Enemies set [(count RydHQB_Enemies), _x]}};
	...
	if (RydHQB_Init) then {[RydHQB] spawn B_Desperado};
	...
	if (RydHQB_Flee) then
		...
		if not ((count RydHQB_Friends) == 0) then {_midCow = _AllCow/(count RydHQB_Friends)};
			...
			if (((random ((20 + (RydHQB_Morale/5))/_AllPanic)) < _cowF) and ((random 100) > (100/(_AllPanic + 1)))) then 
				...
				if (RydHQB_DebugII) then {_i = [(getPosATL (vehicle (leader _x))),_x,"markPanic","ColorYellow","ICON","mil_dot","A!","A!",[0.5,0.5]] call RYD_Mark};
				...
				if (RydHQB_Surr) then
					...
					if ((random 100) > 50) then
						{
						if (RydHQB_DebugII) then {_i setMarkerColor "ColorPink";_i setMarkerText "A!!!"};
						_isCaptive = _x getVariable ("isCaptive" + (str _x));
						if (isNil "_isCaptive") then {_isCaptive = false};
