...
if (_ammo == 0) exitwith {RydHQB_Roger = true};
...
if ((_busy) and (_unitG in RydHQB_AirInDef)) exitwith {RydHQB_Roger = true};
...
RydHQB_AirInDef = RydHQB_AirInDef + [_unitG];
...
while {not (_endThis)} do
	...
	if ((RydHQB_Debug) or (isPlayer (leader _unitG))) then 
		{
		_i = [_DefPos,_unitG,"markDef","ColorBrown","ICON","mil_dot","Air B"," - DEFEND AREA"] call RYD_Mark
		};
	...
	if (_unitG in RydHQB_BAirG) then 
		{
		_chosen = GrpNull;
		_dstM = 5000;

		_chosen = ([_Spot,RydHQB_KnEnemiesG,0] call RYD_CloseEnemyB) select 2;
...
if not (_alive) exitwith {if ((RydHQB_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markDef" + (str _unitG))};RydHQB_AirInDef = RydHQB_AirInDef - [_unitG]};

if ((isPlayer (leader _unitG)) and not (isMultiplayer)) then {(leader _unitG) removeSimpleTask _task};

sleep 30;

if ((RydHQB_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markDef" + (str _unitG))};
