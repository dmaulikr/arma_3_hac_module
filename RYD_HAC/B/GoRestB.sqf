...
if not (isNull _AV) then
	...
	if not (_GDV == _unitG) then
		{
		if not (_GDV in RydHQB_Exhausted) then
			{
	...
	else
		{
			...
			if not ((group _x) == _unitG) then 
				{
				if not ((group _x) in RydHQB_Exhausted) then
					{
					[_x] ordergetIn false;
					unassignVehicle _x
					}
...
if (_unitG in RydHQB_Garrison) exitwith {RydHQB_Exhausted = RydHQB_Exhausted - [_unitG]};
...
_Xpos = ((position leaderHQB) select 0) + (random 500) - 250;
_Ypos = ((position leaderHQB) select 1) + (random 500) - 250;
...
if not (isNil "RydHQB_RestDecoy") then
	{
	_isDecoy = true;

	_tRadius = (triggerArea RydHQB_RestDecoy) select 0;

	if ((random 100) >= RydHQB_RDChance) exitWith {_isDecoy = false};

	_tPos = position RydHQB_RestDecoy;
	_enemyMatters = (triggerArea RydHQB_RestDecoy) select 3;
...
waituntil
	...
	(not (_isWater) and ((isNull (leaderHQB FindNearestEnemy [_posX,_posY])) or (((leaderHQB FindNearestEnemy [_posX,_posY]) distance [_posX,_posY]) >= 500) or not (_enemyMatters)) or (_counter > 30))
	};

if ((_counter > 30) or (not (isNull (leaderHQB FindNearestEnemy [_posX,_posY])) and (((leaderHQB FindNearestEnemy [_posX,_posY]) distance [_posX,_posY]) < 500) and (_enemyMatters))) then {_posX = ((position leaderHQB) select 0) + (random 500) - 250;_posY = ((position leaderHQB) select 1) + (random 500) - 250};

_isWater = surfaceIsWater [_posX,_posY];
if ((_isWater) or (not (isNull (leaderHQB FindNearestEnemy [_posX,_posY])) and (((leaderHQB FindNearestEnemy [_posX,_posY]) distance [_posX,_posY]) < 500) and (_enemyMatters))) exitwith {_unitG setVariable [("Resting" + (str _unitG)),false,true];_unitG setVariable [("Busy" + (str _unitG)), false, true];RydHQB_Exhausted = RydHQB_Exhausted - [_unitG]};
...
if ((RydHQB_Debug) or (isPlayer (leader _unitG))) then
	{
	_i = [[_posX,_posY],_unitG,"markRest","ColorGreen","ICON","mil_triangle","B"," - REST & REGROUP",[0.5,0.5]] call RYD_Mark
	};
...
RydHQB_Exhausted = RydHQB_Exhausted - [_unitG];
