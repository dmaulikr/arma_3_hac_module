...
_dX = (_PosObj1 select 0) - ((getPosATL leaderHQF) select 0);
_dY = (_PosObj1 select 1) - ((getPosATL leaderHQF) select 1);
...
_Armor = RydHQF_LArmorG + RydHQF_HArmorG;

if (_unitG in _Armor) then {_distance2 = 700};
if (_unitG in RydHQF_AirG) then {_distance2 = 800};
...
_posX = ((getPosATL leaderHQF) select 0) + _dXb + (random 200) - 100;
_posY = ((getPosATL leaderHQF) select 1) + _dYb + (random 200) - 100;
...
if ((RydHQF_Debug) or (isPlayer (leader _unitG))) then 
	{
	_i = [[_posX,_posY],_unitG,"markAttack","ColorRed","ICON","mil_dot","Snp F"," - ATTACK"] call RYD_Mark
	};
...
if not (_alive) exitwith {[_Trg,"SnpAttacked"] call RYD_VarReductor;if ((RydHQF_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markAttack" + str (_unitG))}};
...
if (_unitG in RydHQF_Garrison) then
	{
	if (isPlayer (leader _unitG)) then
		{
		if not (isMultiplayer) then
...
_pass orderGetIn true;//_countAv = count RydHQF_AttackAv;
RydHQF_AttackAv = RydHQF_AttackAv + [(_unitG)];
_unitG setVariable [("Busy" + (str _unitG)),false];
