...
if (RydHQF_DebugII) then
	{
	waitUntil 
		{
		sleep 1;
		not (isNil "RydHQF_KnEnemiesG")	
		};
...
while {not (isNull RydHQF)} do
	{
	_LCU = RydHQF_Friends - (RydHQF_NavalG + RydHQF_SupportG + RydHQF_ArtG + (RydHQF_NCCargoG - (RydHQF_NCrewInfG - RydHQF_SupportG)));
	...
	foreach (RydHQF_Friends - RydHQF_AirG);
