...
while {not ((isNull RydHQF) or (RydHQF_Surrender))} do
	...
	if not (isNil ("RydHQF_CivF")) then {_civF = RydHQF_CivF};
		...
		if not ((isNull (leaderHQF)) and not (isNull _x) and (alive (leaderHQF)) and (alive (leader _x)) and not (_isCaptive)) then
			...
			if ((RydHQF_Front) and ((side _x) getFriend (side RydHQF) < 0.6) and ((getPosATL (vehicle (leader _x))) in FrontA) and not (_isCiv)) then {if not (_x in RydHQF_Enemies) then {RydHQF_Enemies set [(count RydHQF_Enemies), _x]}};
	...
	if (RydHQF_Init) then {[RydHQF] spawn F_Desperado};
	...
	if (RydHQF_Flee) then
		...
		if not ((count RydHQF_Friends) == 0) then {_midCow = _AllCow/(count RydHQF_Friends)};
			...
			if (((random ((20 + (RydHQF_Morale/5))/_AllPanic)) < _cowF) and ((random 100) > (100/(_AllPanic + 1)))) then 
				...
				if (RydHQF_DebugII) then {_i = [(getPosATL (vehicle (leader _x))),_x,"markPanic","ColorYellow","ICON","mil_dot","A!","A!",[0.5,0.5]] call RYD_Mark};
				...
				if (RydHQF_Surr) then
					...
					if ((random 100) > 50) then
						{
						if (RydHQF_DebugII) then {_i setMarkerColor "ColorPink";_i setMarkerText "A!!!"};
						_isCaptive = _x getVariable ("isCaptive" + (str _x));
						if (isNil "_isCaptive") then {_isCaptive = false};
