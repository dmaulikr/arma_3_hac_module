 ...
 * Returns:
 * Arguments:
 * Example: [_gp,_pos,_tp,_beh,_CM,_spd,_sts,_crr,_rds,_TO,_frm] call RYD_WPadd;
 */
if (isNil("RYD_WPadd")) then {
	RYD_WPadd =	{
		private	[
			"_gp","_pos","_tp","_beh","_CM","_spd","_sts","_crr","_rds","_TO",
			"_frm","_wp","_vh","_topArr","_fFactor","_posX","_posY","_isWater",
			"_wpn","_addedpath","_assVeh","_wps","_isAir","_sPoint","_dst",
			"_dstFirst","_mPoints","_num","_actDst","_angle","_mPoint",
			"_topPoints","_sPosX","_sPosY","_sUrban","_sForest","_sHills",
			"_sFlat","_sSea","_sGr","_count","_friendly","_opt","_j",
			"_samplePos","_sRoads,","_lastDistance","_dstCheck","_pfAll",
			"_sRoads","_mpl","_frds"
		];
		_pfAll = true;
		_gp = _this select 0;
		...
		if (RydHQ_Rush) then {
			if (_spd == "LIMITED") then	{_spd = "NORMAL";};
			if (_beh == "SAFE") then {_beh = "AWARE";};
		};
		...
		if ((RydHQ_PathFinding > 0) and (_pfAll)) then {
			...
			if not (_isAir) then {
				...
				if (_dst > RydHQ_PathFinding) then {
					_dstFirst = _dst;
					_mPoints = [];
					while {(_dst > RydHQ_PathFinding)} do {_dst = floor (_dst/2);};
					_dst = _dst * 2;
