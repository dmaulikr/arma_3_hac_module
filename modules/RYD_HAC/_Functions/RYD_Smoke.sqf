...
if (isNil("RYD_Smoke")) then {
	RYD_Smoke =	{
		...
		{	if (((vehicle _x) == _x) and not (isPlayer _x)) then {
				if (("SmokeShell" in (magazines _x)) or	("SmokeShellYellow" in (magazines _x)) or ("SmokeShellRed" in (magazines _x)) or
					("SmokeShellBlue" in (magazines _x)) or	("SmokeShellPurple" in (magazines _x)) or ("SmokeShellOrange" in (magazines _x)) or
