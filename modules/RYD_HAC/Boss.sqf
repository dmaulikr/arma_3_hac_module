...
{
	switch (_x) do {
		case (leaderHQ) : {waitUntil {sleep 0.1;not (isNil "RydHQ_readyForBB")}};
		case (leaderHQB) : {waitUntil {sleep 0.1;not (isNil "RydHQB_readyForBB")}};
		case (leaderHQC) : {waitUntil {sleep 0.1;not (isNil "RydHQC_readyForBB")}};
		case (leaderHQD) : {waitUntil {sleep 0.1;not (isNil "RydHQD_readyForBB")}};
		case (leaderHQE) : {waitUntil {sleep 0.1;not (isNil "RydHQE_readyForBB")}};
		case (leaderHQF) : {waitUntil {sleep 0.1;not (isNil "RydHQF_readyForBB")}};
		case (leaderHQG) : {waitUntil {sleep 0.1;not (isNil "RydHQG_readyForBB")}};
		case (leaderHQH) : {waitUntil {sleep 0.1;not (isNil "RydHQH_readyForBB")}};
	}
} foreach _BBHQs;
...
*/
	...
	_ForcesRep = [_BBHQs] call RYD_ForceAnalyze;
...
if ((true) and (true)) then
	{
		...
		if (RydBB_Debug) then
			...
			{
			_isLeft = ([(getPosATL _x),_ArmyPos,_attackAxis] call RYD_WhereIs) select 0;
		...
		if (((RydBBa_SimpleDebug) and (_BBSide == "A")) or ((RydBBb_SimpleDebug) and (_BBSide == "B"))) then {[_BBHQs,_BBSide] spawn RYD_BBSimpleD};
		};
...

			{
				...
				if (_aliveHQ) then
					...
					if (_pathDone) then
						...
						[_x,_tgtsAround,_tObj1,_tObj2,_tObj3,_tObj4,_BBHQs,_HQpos,_front,_secsAround,_goingReserve,_BBSide] spawn RYD_ExecutePath;
	...
	*/
	...
	if (_BBCycle == 1) then {[_strArea,_BBSide,(_BBHQs select 0)] spawn RYD_ObjectivesMon};
...
/*
//kto gdzie: ilosc sektorow, celow i obecnosc wroga -> liczebnosc, topografia i ilosc sektor?w oraz cel?w, sklad sil wroga -> sklad armii
