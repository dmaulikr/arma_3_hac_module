...
if (_unitG in RydHQC_Garrison) exitwith {RydHQC_FlankAv = RydHQC_FlankAv - [_unitG]};
...
if (_isWater) exitwith {_unitG setVariable [("Busy" + (str _unitG)),false];RydHQC_FlankAv = RydHQC_FlankAv - [_unitG]};
...
if ((_ammo > 0) and not (_busy)) then
	...
	if ((isPlayer (leader _unitG)) and not (isMultiplayer)) then {(leader _unitG) removeSimpleTask _task};
	RydHQC_FlankAv = RydHQC_FlankAv - [_unitG];
	_unitG setVariable [("Busy" + _unitvar), false];

	if ((RydHQC_Debug) or (isPlayer (leader _unitG))) then {_i4 setMarkerColor "ColorBlue"};
	sleep 30;
	if ((RydHQC_Debug) or (isPlayer (leader _unitG))) then
		{
			{
