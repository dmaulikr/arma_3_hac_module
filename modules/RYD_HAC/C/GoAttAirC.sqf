...
_posX = ((getPos leaderHQC) select 0) + _dXb;
_posY = ((getPos leaderHQC) select 1) + _dYb;
...
if ((RydHQC_Debug) or (isPlayer (leader _unitG))) then
	{
	_i = [[_posX,_posY],_unitG,"markAttack","ColorRed","ICON","mil_dot","Air C"," - ATTACK"] call RYD_Mark
	};
...
if (_unitG in RydHQC_BAirG) then {deleteVehicle _lasT};
...
_countAv = count RydHQC_AttackAv;
RydHQC_AttackAv = RydHQC_AttackAv + [(_unitG)];
