...
RydHQC_ReconAv = RydHQC_ReconAv - [_unitG];

_UL = leader _unitG;
_PosLand = position _UL;
_nothing = true;
_End = [((position leaderHQC) select 0) + (random 400) - 200,((position leaderHQC) select 1) + (random 400) - 200];
_rd = 200;
...
if (_isWater) exitwith {RydHQC_ReconAv = RydHQC_ReconAv + [(_unitG)];_unitG setVariable [("Busy" + (str _unitG)),false]};
...
if (not (isNull _GDV) and (_GDV in (RydHQC_NCCargoG + RydHQC_AirG)) and (_NeNMode)) then {_LX1 = (position _UL) select 0;_LY1 = (position _UL) select 1;_halfway = true};
...
if ((isNull _AV) and (([_posX,_posY] distance _UL) > 1500) and not (isPlayer (leader _unitG))) then
	...
	_beh = "SAFE";
	if (_unitG in RydHQC_RAirG) then {_beh = "CARELESS"};
	_spd = "LIMITED";
	_TO = [0,0,0];
...
_tp = "MOVE";
//if (not (isNull _AV) and (_unitG in RydHQC_NCrewInfG)) then {_tp = "UNLOAD"};
_beh = "SAFE";
if (not (isNull _AV) and (_GDV in RydHQC_AirG) or (_unitG in RydHQC_RAirG)) then {_beh = "CARELESS"};
_spd = "NORMAL";
...
_cause = [_unitG,6,true,600,50,[RydHQC_AirG,RydHQC_KnEnemiesG],false] call RYD_Wait;
...
_cause = [_unitG,6,true,250,30,[RydHQC_AirG,RydHQC_KnEnemiesG],false] call RYD_Wait;
_alive = _cause select 1;

if not (_alive) exitwith {if ((RydHQC_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markRecon" + str (_unitG))}};
if ((count (waypoints _unitG)) >= 1) then {deleteWaypoint [_unitG, 1]};

if (_stage >= 4) then {RydHQC_ReconDone = true};
...
if (_unitG in RydHQC_FOG) then
	...
	_cause = [_unitG,6,true,150,120,[RydHQC_AirG,RydHQC_KnEnemiesG],false] call RYD_Wait;
	_timer = _cause select 0;
	_alive = _cause select 1;
	_enemy = _cause select 2
	};
...
while {(_nothing)} do
	...
	if (((not (isNull (_UL findNearestEnemy _UL)) or (_timer2 > 4)) and not (isNull _unitG) and not (_unitG in RydHQC_FOG)) or ((_timer2 > 40) and not (isNull _unitG))) then
		{
		if (_unitG in RydHQC_NCrewInfG) then {_pass orderGetIn true};
		sleep 15;
		_rd = 0;
		...
		if ((RydHQC_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markRecon" + str (_unitG))};
		_nothing = false;
		...
		if (_alive) then
			{
			if (_unitG in (RydHQC_reconG + RydHQC_FOG + RydHQC_snipersG + RydHQC_InfG)) then {RydHQC_ReconAv = RydHQC_ReconAv + [_unitG];};
