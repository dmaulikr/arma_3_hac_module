...
if (_ammo == 0) exitwith {RydHQD_Roger = true};
...
if ((_busy) and (_unitG in RydHQD_AirInDef)) exitwith {RydHQD_Roger = true};
...
RydHQD_AirInDef = RydHQD_AirInDef + [_unitG];
...
while {not (_endThis)} do
	...
	if ((RydHQD_Debug) or (isPlayer (leader _unitG))) then
		{
		_i = [_DefPos,_unitG,"markDef","ColorBrown","ICON","mil_dot","Air D"," - DEFEND AREA"] call RYD_Mark
		};
	...
	if (_unitG in RydHQD_BAirG) then
		{
		_chosen = GrpNull;
		_dstM = 5000;

		_chosen = ([_Spot,RydHQD_KnEnemiesG,0] call RYD_CloseEnemyB) select 2;
...
if not (_alive) exitwith {if ((RydHQD_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markDef" + (str _unitG))};RydHQD_AirInDef = RydHQD_AirInDef - [_unitG]};

if ((isPlayer (leader _unitG)) and not (isMultiplayer)) then {(leader _unitG) removeSimpleTask _task};

sleep 30;

if ((RydHQD_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markDef" + (str _unitG))};
