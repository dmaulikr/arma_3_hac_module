		...
		while {((count _airDef) > (count RydHQD_AirInDef))} do
			...
			if not (_isDef) then
				...
				if not (_x in RydHQD_Def) then
					...
					switch (true) do
						{
						case (_x in RydHQD_InfG) : {_expression = "(1 + (2 * Houses)) * (1 + (1.5 * Forest)) * (1 + Trees) * (1 - Meadow) * (1 - (10 * sea))"};
						case (not (_x in RydHQD_InfG)) : {_expression = "(1 + (2 * Meadow)) * (1 - Forest) * (1 - (0.5 * Trees)) * (1 - (10 * sea))"};
						};
