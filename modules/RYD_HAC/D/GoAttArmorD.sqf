...
if ((RydHQD_Debug) or (isPlayer (leader _unitG))) then
	{
	_i = [[_posX,_posY],_unitG,"markAttack","ColorRed","ICON","mil_dot","Arm D"," - ATTACK"] call RYD_Mark;
	};
...
if ((RydHQD_Debug) or (isPlayer (leader _unitG))) then {deleteMarker _i};

_countAv = count RydHQD_AttackAv;
RydHQD_AttackAv = RydHQD_AttackAv + [(_unitG)];
_unitG setVariable [("Busy" + (str _unitG)),false];
