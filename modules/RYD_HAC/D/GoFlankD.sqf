...
if (_unitG in RydHQD_Garrison) exitwith {RydHQD_FlankAv = RydHQD_FlankAv - [_unitG]};
...
_dX = _BEposX - ((getPosATL leaderHQD) select 0);
_dY = _BEposY - ((getPosATL leaderHQD) select 1);
...
_BorHQD = leaderHQD distance _BEnemyPos;
...
_posXWP1 = ((getPosATL leaderHQD) select 0) + _FlankPosX + _safeX1 + (random 200) - 100;
_posYWP1 = ((getPosATL leaderHQD) select 1) + _FlankPosY + _safeY1 + (random 200) - 100;
...
if (_isWater) exitwith {_unitG setVariable [("Busy" + (str _unitG)),false];RydHQD_FlankAv = RydHQD_FlankAv - [_unitG]};
...
if ((leaderHQD distance [_posXWP1,_posYWP1]) > (leaderHQD distance [_posXWP2,_posYWP2])) then
	{
...
if ((_ammo > 0) and not (_busy)) then
	...
	if ((RydHQD_Debug) or (isPlayer (leader _unitG))) then
		{
		_i1 = [[_posXWP1,_posYWP1],_unitG,"markFlank1","ColorOrange","ICON","mil_dot","Fl1 D"," - FLANK 1"] call RYD_Mark;
		_i2 = [[_posXWP2,_posYWP2],_unitG,"markFlank2","ColorOrange","ICON","mil_dot","Fl2 D"," - FLANK 2"] call RYD_Mark;
		_i3 = [[_posXWP3,_posYWP3],_unitG,"markFlank3","ColorOrange","ICON","mil_dot","Fl3 D"," - FLANK 3"] call RYD_Mark;
		_i4 = [[_posXWP4,_posYWP4],_unitG,"markFlank4","ColorOrange","ICON","mil_dot","Fl4 D"," - FLANKING ATTACK"] call RYD_Mark
		};

	_CargoCheck = _unitG getvariable ("CC" + _unitvar);
	if (isNil ("_CargoCheck")) then {_unitG setVariable [("CC" + _unitvar), false]};
	_AV = assignedVehicle _UL;
	if ((RydHQD_CargoFind > 0) and (isNull _AV) and (([_posXWP4,_posYWP4] distance (vehicle _UL)) > 1000)) then {[_unitG] spawn D_SCargo } else {_unitG setVariable [("CC" + _unitvar), true]};
	if (RydHQD_CargoFind > 0) then
		{
	...
	if (not (isNull _AV) and (RydHQD_CargoFind > 0)) then
		{
		...
		if (RydHQD_LZ) then {deleteVehicle (_AV getVariable ["TempLZ",objNull])};
		_timer = _cause select 0
		};

	if ((isPlayer (leader _unitG)) and not (isMultiplayer)) then {(leader _unitG) removeSimpleTask _task};
	if ((isNull (leader (_this select 0))) or (_timer > 900)) exitwith {if ((RydHQD_Debug) or (isPlayer (leader _unitG))) then {{deleteMarker _x} foreach [_i1,_i2,_i3,_i4]};if not (isNull _GDV) then {[_GDV, (currentWaypoint _GDV)] setWaypointPosition [getPosATL (vehicle (leader _GDV)), 1];_GDV setVariable [("Busy" + _unitvar), false];}};
	...
	_beh = "SAFE";
	if (not (isNull _AV) and (_GDV in RydHQD_AirG)) then {_beh = "CARELESS"};
	_TO = [0,0,0];
	...
	if not (((group _DAV) == (group _UL)) or (isNull (group _DAV))) then
		...
		_cause = [_GDV,6,true,300,30,[RydHQD_AirG,RydHQD_KnEnemiesG],true] call RYD_Wait;
		_timer = _cause select 0;
		...
		{
		_cause = [_unitG,6,true,300,30,[RydHQD_AirG,RydHQD_KnEnemiesG],false] call RYD_Wait;
		_timer = _cause select 0;
		_alive = _cause select 1;
		_enemy = _cause select 2
		};
	...
	if (not (_alive) and not (_OtherGroup)) exitwith {if ((RydHQD_Debug) or (isPlayer (leader _unitG))) then {{deleteMarker _x} foreach [_i1,_i2,_i3,_i4]}};
	if (isNull (leader (_this select 0))) exitwith {if ((RydHQD_Debug) or (isPlayer (leader _unitG))) then {{deleteMarker _x} foreach [_i1,_i2,_i3,_i4]};if not (isNull _GDV) then {[_GDV, (currentWaypoint _GDV)] setWaypointPosition [getPosATL (vehicle (leader _GDV)), 1];_GDV setVariable [("Busy" + _unitvar), false];}};
	...
	if ((RydHQD_Debug) or (isPlayer (leader _unitG))) then {_i1 setMarkerColor "ColorBlue"};
	...
	if ((RydHQD_Debug) or (isPlayer (leader _unitG))) then {_i2 setMarkerColor "ColorBlue"};

	_tp = "MOVE";
	//if (not (isNull _AV) and (_unitG in RydHQD_NCrewInfG) and not ((_GDV == _unitG) or (_GDV in RydHQD_AirG))) then {_tp = "UNLOAD"};
	_grp = _unitG;
	if not (isNull _AV) then {_grp = _GDV};
	_beh = "AWARE";
	_lz = objNull;
	if (not (isNull _AV) and (_GDV in RydHQD_AirG)) then
		{
		_beh = "CARELESS";
		if (RydHQD_LZ) then
			{
	...
	//if (((group (assigneddriver _AV)) in RydHQD_AirG) and (_unitG in RydHQD_NCrewInfG)) then {_sts = ["true","(vehicle this) land 'GET OUT';deletewaypoint [(group this), 0]"]};
	_TO = [0,0,0];
	...
	if not (((group _DAV) == (group _UL)) or (isNull (group _DAV))) then
		...
		_cause = [_GDV,6,true,400,30,[RydHQD_AirG,RydHQD_KnEnemiesG],false] call RYD_Wait;
		_timer = _cause select 0;
		_alive = _cause select 1;
		_enemy = _cause select 2
		}
		...
		{
		_cause = [_unitG,6,true,400,30,[RydHQD_AirG,RydHQD_KnEnemiesG],false] call RYD_Wait;
		_timer = _cause select 0;
		_alive = _cause select 1;
		_enemy = _cause select 2
		};
	...
	if not ((_GDV == _unitG) or (isNull _GDV)) then
		...
		{
		if (_unitG in RydHQD_NCrewInfG) then {_pass orderGetIn false};
		};

	_DAV = assigneddriver _AV;
	_GDV = group _DAV;

	if (not (isNull _AV) and (RydHQD_CargoFind > 0) and (_unitG in RydHQD_NCrewInfG)) then
		{
	...
	if (RydHQD_LZ) then {deleteVehicle _lz};
	...
	if ((isNull (leader (_this select 0))) or (_timer > 240)) exitwith {if ((RydHQD_Debug) or (isPlayer (leader _unitG))) then {{deleteMarker _x} foreach [_i1,_i2,_i3,_i4]};if not (isNull _GDV) then {[_GDV, (currentWaypoint _GDV)] setWaypointPosition [getPosATL (vehicle (leader _GDV)), 1];_GDV setVariable [("Busy" + _unitvar), false];_pass orderGetIn true}};

	_unitvar = str _GDV;
	_timer = 0;
	if (not (isNull _GDV) and (_GDV in RydHQD_AirG) and not (isPlayer (leader _GDV))) then
		{
	...
	if ((RydHQD_Debug) or (isPlayer (leader _unitG))) then {_i3 setMarkerColor "ColorBlue"};
	...
	if ((isPlayer (leader _unitG)) and not (isMultiplayer)) then {(leader _unitG) removeSimpleTask _task};
	RydHQD_FlankAv = RydHQD_FlankAv - [_unitG];
	_unitG setVariable [("Busy" + _unitvar), false];

	if ((RydHQD_Debug) or (isPlayer (leader _unitG))) then {_i4 setMarkerColor "ColorBlue"};
	sleep 30;
	if ((RydHQD_Debug) or (isPlayer (leader _unitG))) then
		{
			{
