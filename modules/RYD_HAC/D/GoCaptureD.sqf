...
if (isNil ("RydHQD_Obj")) then {_trg = leaderHQD} else {_Trg = RydHQD_Obj};
...
_currentObj = RydHQD_NObj;
_BBProg = RydHQD getVariable ["BBProgress",0];

_unitG setVariable [("Capt" + (str _unitG)),true];
RydHQD_AttackAv = RydHQD_AttackAv - [_unitG];

_UL = leader _unitG;
_nothing = true;

_dX = (_PosObj1 select 0) - ((getPos leaderHQD) select 0);
_dY = (_PosObj1 select 1) - ((getPos leaderHQD) select 1);

_angle = _dX atan2 _dY;

_distance = leaderHQD distance _PosObj1;
_distance2 = 100;
...
_posX = ((getPos leaderHQD) select 0) + _dXb + _dXc + (random 200) -  100;
_posY = ((getPos leaderHQD) select 1) + _dYb + _dYc + (random 200) -  100;
...
if ((RydHQD_Debug) or (isPlayer (leader _unitG))) then
	{
	_i = [[_posX,_posY],_unitG,"markCapture","ColorRed","ICON","mil_dot","Cap D"," - SECURE AREA"] call RYD_Mark
	};

_CargoCheck = _unitG getvariable ("CC" + _unitvar);
if (isNil ("_CargoCheck")) then {_unitG setVariable [("CC" + _unitvar), false]};
_AV = assignedVehicle _UL;
if ((RydHQD_CargoFind > 0) and (isNull _AV) and (([_posX,_posY] distance (vehicle _UL)) > 1000)) then {[_unitG] spawn D_SCargo } else {_unitG setVariable [("CC" + _unitvar), true]};
...
if ((isNull (leader (_this select 0))) or (_timer > 900)) exitwith
	...
	_isAttacked = _isAttacked - 1;
	_Trg setVariable [("Capturing" + (str _Trg)),[_isAttacked,_amountC]];
	if ((RydHQD_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markCapture" + str (_unitG))};
...
_eClose1 = [[_posX,_posY],RydHQD_KnEnemiesG,300] call RYD_CloseEnemyB;
...
if not (isNull _NeN) then
	{
	_eClose2 = [_UL,RydHQD_KnEnemiesG,300] call RYD_CloseEnemyB;
	_tooC2 = _eClose2 select 0;
	_dstEM2 = _eClose2 select 1;
	_eClose3 = [leaderHQD,RydHQD_KnEnemiesG,300] call RYD_CloseEnemyB;
...
if (not (isNull _GDV) and (_GDV in (RydHQD_NCCargoG + RydHQD_AirG)) and (_NeNMode) and not (isPlayer (leader _GDV))) then {_LX1 = (position _UL) select 0;_LY1 = (position _UL) select 1;_halfway = true};
...
_BBProgN = RydHQD getVariable ["BBProgress",0];
...
if ((RydHQD_Debug) or (isPlayer (leader _unitG))) then {_i setMarkerColor "ColorBlue"};

_isTaken = false;
_waitTime = RydHQD_ObjHoldTime;
...
if (leaderHQD in (RydBBa_HQs + RydBBb_HQs)) then {_mGain = 0};
...
waitUntil
	{
	sleep 60;
	_BBProgN = RydHQD getVariable "BBProgress";
	if (isNil "_BBProgN") then {_BBProgN = 0};

	if ((RydHQD_NObj == _currentObj) and not (_BBProgN > _BBProg)) then
		{
		_waitTime = _waitTime - 60;

		_AllV0 = _trg nearEntities [["AllVehicles"],RydHQD_ObjRadius1];
		...
		_Civs0 = _trg nearEntities [["Civilian"],RydHQD_ObjRadius1];
		...
		_AllV20 = _trg nearEntities [["AllVehicles"],RydHQD_ObjRadius2];
		...
		if (not (RydHQD_UnlimitedCapt) and (_NearAllies >= RydHQD_CaptLimit) and (_NearEnemies <= (0 + ((RydHQD_Recklessness/(0.5 + RydHQD_Consistency))*10)))) then
			{
			RydHQD_NObj = RydHQD_NObj + 1;
			if ((random 100) < (15*RydHQD_NObj)) then
				{
				RydHQD_FlankingDone = false;
				RydHQD_FlankingInit = false
				};
			RydHQD_Morale = RydHQD_Morale + _mGain;
			_isTaken = true
			};
		...
		{
		if (not (RydHQD_NObj < _currentObj) or (RydBB_Active)) then
			{
			_isTaken = true
			}
		else
...
if ((RydHQD_UnlimitedCapt) or not (_isTaken)) then {_Trg setvariable [("Capturing" + str _Trg),[0,0]]};
...
_all = true;
	...
	{
	if ((_x in RydHQD_AttackAv) and not (_x getVariable ("Busy" + str (_x)))) exitwith {_all = false};
	}
foreach RydHQD_Friends - ((RydHQD_AirG - RydHQD_NCrewInfG) + RydHQD_NavalG + RydHQD_StaticG + RydHQD_SupportG + RydHQD_ArtG + (RydHQD_NCCargoG - (RydHQD_NCrewInfG - RydHQD_SupportG)));
