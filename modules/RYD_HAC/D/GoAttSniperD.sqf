...
_dX = (_PosObj1 select 0) - ((getPosATL leaderHQD) select 0);
_dY = (_PosObj1 select 1) - ((getPosATL leaderHQD) select 1);
...
if (_unitG in RydHQD_AirG) then {_distance2 = 800};
...
_posX = ((getPosATL leaderHQD) select 0) + _dXb + (random 200) - 100;
_posY = ((getPosATL leaderHQD) select 1) + _dYb + (random 200) - 100;
...
if ((RydHQD_Debug) or (isPlayer (leader _unitG))) then
	{
	_i = [[_posX,_posY],_unitG,"markAttack","ColorRed","ICON","mil_dot","Snp D"," - ATTACK"] call RYD_Mark
	};
...
if not (_alive) exitwith {[_Trg,"SnpAttacked"] call RYD_VarReductor;if ((RydHQD_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markAttack" + str (_unitG))}};
