...
RydHQG_ReconAv = RydHQG_ReconAv - [_unitG];

_UL = leader _unitG;
_PosLand = position _UL;
_nothing = true;
_End = [((position leaderHQG) select 0) + (random 400) - 200,((position leaderHQG) select 1) + (random 400) - 200];
_rd = 200;
...
if (_isWater) exitwith {RydHQG_ReconAv = RydHQG_ReconAv + [(_unitG)];_unitG setVariable [("Busy" + (str _unitG)),false]};
...
if ((RydHQG_Debug) or (isPlayer (leader _unitG))) then
	{
	_i = [[_posX,_posY],_unitG,"markRecon","ColorRed","ICON","mil_dot","Rec G"," - NON-COMBAT RECON"] call RYD_Mark;
	};
...
if ((isNull _AV) and (([_posX,_posY] distance _UL) > 1500) and not (isPlayer (leader _unitG))) then
	...
	_beh = "SAFE";
	if (_unitG in RydHQG_RAirG) then {_beh = "CARELESS"};
	_spd = "LIMITED";
	_TO = [0,0,0];
...
_beh = "SAFE";
if (not (isNull _AV) and (_GDV in RydHQG_AirG) or (_unitG in RydHQG_RAirG)) then {_beh = "CARELESS"};
_spd = "NORMAL";
...
_cause = [_unitG,6,true,600,50,[RydHQG_AirG,RydHQG_KnEnemiesG],false] call RYD_Wait;
...
_cause = [_unitG,6,true,250,30,[RydHQG_AirG,RydHQG_KnEnemiesG],false] call RYD_Wait;
_alive = _cause select 1;

if not (_alive) exitwith {if ((RydHQG_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markRecon" + str (_unitG))}};
if ((count (waypoints _unitG)) >= 1) then {deleteWaypoint [_unitG, 1]};

if (_stage >= 4) then {RydHQG_ReconDone = true};
...
if (_unitG in RydHQG_FOG) then
	...
	_cause = [_unitG,6,true,150,120,[RydHQG_AirG,RydHQG_KnEnemiesG],false] call RYD_Wait;
	_timer = _cause select 0;
	_alive = _cause select 1;
	_enemy = _cause select 2
	};
...
while {(_nothing)} do
	...
	if (((not (isNull (_UL findNearestEnemy _UL)) or (_timer2 > 4)) and not (isNull _unitG) and not (_unitG in RydHQG_FOG)) or ((_timer2 > 40) and not (isNull _unitG))) then
		{
		if (_unitG in RydHQG_NCrewInfG) then {_pass orderGetIn true};
		sleep 15;
		_rd = 0;
		if (_unitG in RydHQG_AirG) then {_End = _PosLand;_rd = 0} else {_End = [((position leaderHQG) select 0) + (random 400) - 200,((position leaderHQG) select 1) + (random 400) - 200];_isWater = surfaceIsWater _End;if (_isWater) then {_End = [((position leaderHQG) select 0) + (random 40) - 20,((position leaderHQG) select 1) + (random 40) - 20]}};
		...
		if ((RydHQG_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markRecon" + str (_unitG))};
		_nothing = false;
		...
		if (_alive) then
			{
			if (_unitG in (RydHQG_reconG + RydHQG_FOG + RydHQG_snipersG + RydHQG_InfG)) then {RydHQG_ReconAv = RydHQG_ReconAv + [_unitG];};
