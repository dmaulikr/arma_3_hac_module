...
_DefPos = _this select 1;if (_unitG in RydHQG_Garrison) exitwith {RydHQG_DefSpot = RydHQG_DefSpot - [_unitG];RydHQG_GoodSpots = RydHQG_GoodSpots + [_DefPos];RydHQG_Roger = true};
...
if ((_unitG in RydHQG_NCCargoG) and ((count (units _unitG)) <= 1)) then
	{
	_PosX = ((position leaderHQG) select 0) + (random 200) - 100;
	_PosY = ((position leaderHQG) select 1) + (random 200) - 100;
	_DefPos = [_posX,_posY]
	};
...
if ((RydHQG_Debug) or (isPlayer (leader _unitG))) then
	{
	_i = [_DefPos,_unitG,"markDef","ColorRedAlpha","ICON","mil_dot","DRes G"," - DEFEND POSITION"] call RYD_Mark
	};
...
_dX = 2000 * (sin RydHQG_Angle);
_dY = 2000 * (cos RydHQG_Angle);
...
if ((RydHQG_Debug) or (isPlayer (leader _unitG))) then
	{
	_i = [_TED,_unitG,"markWatch","ColorGreenAlpha","ICON","mil_dot","G","G",[0.2,0.2]] call RYD_Mark
	};
...
waituntil
	...
	if (_sumD > 0) then {_dstAv = _sumD/(count RydHQG_Friends)};
	...
	if ((count _tooClose) > 0) then
		{
		_HQinDanger = true;
		_HQsupporting = RydHQG getVariable "SPRTD";
		if (isNil "_HQsupporting") then {_HQsupporting = 0}
		};

	_HQchance = (80 - (_HQsupporting * 20))/(1 + (RydHQG_Recklessness * 0.5));
	...
	if ((_HQinDanger) and ((random 100) < _HQchance)) then
		...
		_unitG setVariable ["Reinforcing",RydHQG];
		RydHQG setVariable ["SPRTD",_HQsupporting + 1]
		}
	else
		{
		_ToReinf = RydHQG_Friends - (RydHQG_AirG + RydHQG_DefRes);
	...
	if not (isNull _chosen) then
		...
		if ((RydHQG_Debug) or (isPlayer (leader _unitG))) then
			{
			_i = [_RnfP,_unitG,"markReinf","ColorRedAlpha","ICON","mil_dot","Reinf G"," - REINFORCE POSITION",[0.3,0.3]] call RYD_Mark
			}
		};


	if not (_unitG getVariable "Defending") then {_endThis = true};
	if ((isNull _unitG) or (isNull RydHQG)) then {_endThis = true;_alive = false};
...
if ((RydHQG_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markDef" + (str _unitG));deleteMarker ("markWatch" + (str _unitG));deleteMarker ("markReinf" + (str _unitG))};
