...
if (RydHQG_DebugII) then
	{
	waitUntil
		{
		sleep 1;
		not (isNil "RydHQG_KnEnemiesG")
		};
...
while {not (isNull RydHQG)} do
	{
	_LCU = RydHQG_Friends - (RydHQG_NavalG + RydHQG_SupportG + RydHQG_ArtG + (RydHQG_NCCargoG - (RydHQG_NCrewInfG - RydHQG_SupportG)));
	...
	foreach (RydHQG_Friends - RydHQG_AirG);
