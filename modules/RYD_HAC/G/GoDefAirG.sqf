...
if (_ammo == 0) exitwith {RydHQG_Roger = true};
...
if ((_busy) and (_unitG in RydHQG_AirInDef)) exitwith {RydHQG_Roger = true};
...
RydHQG_AirInDef = RydHQG_AirInDef + [_unitG];
...
while {not (_endThis)} do
	...
	if (_unitG in RydHQG_BAirG) then
		...
		_chosen = ([_Spot,RydHQG_KnEnemiesG,0] call RYD_CloseEnemyB) select 2;
...
if not (_alive) exitwith {if ((RydHQG_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markDef" + (str _unitG))};RydHQG_AirInDef = RydHQG_AirInDef - [_unitG]};

if ((isPlayer (leader _unitG)) and not (isMultiplayer)) then {(leader _unitG) removeSimpleTask _task};

sleep 30;

if ((RydHQG_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markDef" + (str _unitG))};
