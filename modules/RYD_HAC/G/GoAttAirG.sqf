...
_posX = ((getPos leaderHQG) select 0) + _dXb;
_posY = ((getPos leaderHQG) select 1) + _dYb;
...
if ((RydHQG_Debug) or (isPlayer (leader _unitG))) then
	{
	_i = [[_posX,_posY],_unitG,"markAttack","ColorRed","ICON","mil_dot","Air G"," - ATTACK"] call RYD_Mark
	};
...
if (_unitG in RydHQG_BAirG) then {deleteVehicle _lasT};
...
_countAv = count RydHQG_AttackAv;
RydHQG_AttackAv = RydHQG_AttackAv + [(_unitG)];
