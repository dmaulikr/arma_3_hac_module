...
_Xpos = ((((position leaderHQE) select 0) + ((position RydHQE_Obj) select 0))/2) + (random 100) - 50;
_Ypos = ((((position leaderHQE) select 1) + ((position RydHQE_Obj) select 1))/2) + (random 100) - 50;
...
if (not (isNil "RydHQE_IdleDecoy") and not (_unitG in RydHQE_SupportG)) then
	{
	_isDecoy = true;

	_tRadius = (triggerArea RydHQE_IdleDecoy) select 0;

	if ((random 100) >= RydHQE_IDChance) exitWith {_isDecoy = false};

	_tPos = position RydHQE_IdleDecoy;
	_enemyMatters = (triggerArea RydHQE_IdleDecoy) select 3;

	_posX = (_tPos select 0) + (random (2 * _tRadius)) - (_tRadius);
	_posY = (_tPos select 1) + (random (2 * _tRadius)) - (_tRadius);
	};

if (not (isNil "RydHQE_SupportDecoy") and (_unitG in RydHQE_SupportG)) then
	{
	_tRadius = (triggerArea RydHQE_SupportDecoy) select 0;

	if ((random 100) >= RydHQE_SDChance) exitWith {_isDecoy = false};

	_tPos = position RydHQE_SupportDecoy;
	_enemyMatters = (triggerArea RydHQE_SupportDecoy) select 3;
...
if not (_isDecoy) then
	...
	RydHQE_Bpoint = _position;

	if (not (_unitG in (RydHQE_SupportG + RydHQE_NCCargoG)) and ((random 100) >= 50) and ((_VLU distance [_Xpos,_Ypos]) > (_VLU distance leaderHQE))) then
		{
		_position = [((position leaderHQE) select 0) + (random 400) - 200,((position leaderHQE) select 1) + (random 400) - 200]
		}
	else
		{
		if (not (_unitG in (RydHQE_NCCargoG - RydHQE_SupportG)) and ((_VLU distance [_Xpos,_Ypos]) < (_VLU distance RydHQE_Obj)) and (_behind)) then
			{
			...
			{
			if not (_unitG in (RydHQE_SupportG + RydHQE_NCCargoG)) then
				{
	...
	if (not (_allowed) and (_unitG in RydHQE_SupportG)) exitwith {};
	...
	switch (true) do
		{
		case (_unitG in (RydHQE_NCrewInfG - RydHQE_SupportG)) : {_expression = "(1 + (2 * Houses)) * (1 + (1.5 * Forest)) * (1 + Trees) * (1 - Meadow) * (1 - (10 * sea))"};
		case (not (_unitG in (RydHQE_NCrewInfG - RydHQE_SupportG))) : {_expression = "(1 + (2 * Meadow)) * (1 - Forest) * (1 - (0.5 * Trees)) * (1 - (10 * sea)) * (1 - (2 * Houses))"};
		};
	...
	if (((_unitG in (RydHQE_NCrewInfG - RydHQE_SupportG - RydHQE_CargoG)) and ((random 100) > (20/0.5 + RydHQE_Activity))) or (((random 100) > (80/0.5 + RydHQE_Activity)) and not (_unitG in RydHQE_SupportG))) then {_patrol = true};

	_sec = false;

	if  ((not (_unitG in RydHQE_NCCargoG) or ((count (units _unitG)) > 1)) and not (_unitG in RydHQE_SupportG) and ((_VLU distance RydHQE_Obj) > (_VLU distance leaderHQE))) then
		{
		_rnd = random 100;

		switch (true) do
			{
			case ((_rnd > 50) and (_rnd <= 75)) : {if not (isNil ("RydHQE_Sec1")) then {_posX = ((position RydHQE_Sec1) select 0) + (random 200) - 100;_posY = ((position RydHQE_Sec1) select 1) + (random 200) - 100};_sec = true};
			case (_rnd > 75) : {if not (isNil ("RydHQE_Sec2")) then {_posX = ((position RydHQE_Sec2) select 0) + (random 200) - 100;_posY = ((position RydHQE_Sec2) select 1) + (random 200) - 100};_sec = true};
			};
		};

	_NR = _pos nearRoads 400;
	_cnt = 0;
	if (not (_patrol) and not (_sec) and not ((random 100) < (20/(0.5 + RydHQE_Fineness))) and not ((count _NR) == 0) and not (_unitG in (RydHQE_SupportG + RydHQE_NCCargoG))) then
		{
		while {(true)} do
			 {
			 _cnt = _cnt + 1;
...
_eClose = [[_posX,_posY],RydHQE_KnEnemiesG,500] call RYD_CloseEnemy;
...
if ((RydHQE_Debug) or (isPlayer (leader _unitG))) then
	...
	_i = [[_posX,_posY],_unitG,"markIdle","ColorRedAlpha","ICON","mil_dot","Res E",_pltxt,[0.5,0.5]] call RYD_Mark;
	};
...
if (_unitG in (RydHQE_NCrewInfG - RydHQE_SupportG)) then {_tp = "MOVE"};
_CM = "YELLOW";
_spd = "LIMITED";
if (_unitG in (RydHQE_SupportG)) then {_CM = "BLUE";_tp = "MOVE";_spd = "NORMAL"};
_sts = ["true","deletewaypoint [(group this), 0];"];
if (_unitG in RydHQE_AirG) then {_sts = ["true", "{(vehicle _x) land 'LAND'} foreach (units (group this)); deletewaypoint [(group this), 0]"]};
...
if not (_alive) exitwith {if ((RydHQE_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markIdle" + str (_unitG))}};

if not (((vehicle _LU) == _LU) and not (isPlayer _LU) and not (isPlayer (driver (assignedvehicle (_LU))))) then {(vehicle _LU) engineOn false};

if ((RydHQE_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markIdle" + str (_unitG))};
