...
if (isNil ("RydHQE_Obj")) then {_trg = leaderHQE} else {_Trg = RydHQE_Obj};
...
_currentObj = RydHQE_NObj;
_BBProg = RydHQE getVariable ["BBProgress",0];
...
_dX = (_PosObj1 select 0) - ((getPos leaderHQE) select 0);
_dY = (_PosObj1 select 1) - ((getPos leaderHQE) select 1);

_angle = _dX atan2 _dY;

_distance = leaderHQE distance _PosObj1;
_distance2 = 100;
...
_posX = ((getPos leaderHQE) select 0) + _dXb + _dXc + (random 200) -  100;
_posY = ((getPos leaderHQE) select 1) + _dYb + _dYc + (random 200) -  100;
...
if ((RydHQE_Debug) or (isPlayer (leader _unitG))) then
	{
	_i = [[_posX,_posY],_unitG,"markCapture","ColorRed","ICON","mil_dot","Cap E"," - SECURE AREA"] call RYD_Mark
	};

_CargoCheck = _unitG getvariable ("CC" + _unitvar);
if (isNil ("_CargoCheck")) then {_unitG setVariable [("CC" + _unitvar), false]};
_AV = assignedVehicle _UL;
if ((RydHQE_CargoFind > 0) and (isNull _AV) and (([_posX,_posY] distance (vehicle _UL)) > 1000)) then {[_unitG] spawn E_SCargo } else {_unitG setVariable [("CC" + _unitvar), true]};
...
if ((isNull (leader (_this select 0))) or (_timer > 900)) exitwith
	...
	_isAttacked = _isAttacked - 1;
	_Trg setVariable [("Capturing" + (str _Trg)),[_isAttacked,_amountC]];
	if ((RydHQE_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markCapture" + str (_unitG))};
...
_eClose1 = [[_posX,_posY],RydHQE_KnEnemiesG,300] call RYD_CloseEnemyB;
...
if not (isNull _NeN) then
	{
	_eClose2 = [_UL,RydHQE_KnEnemiesG,300] call RYD_CloseEnemyB;
	_tooC2 = _eClose2 select 0;
	_dstEM2 = _eClose2 select 1;
	_eClose3 = [leaderHQE,RydHQE_KnEnemiesG,300] call RYD_CloseEnemyB;
...
if (not (isNull _GDV) and (_GDV in (RydHQE_NCCargoG + RydHQE_AirG)) and (_NeNMode) and not (isPlayer (leader _GDV))) then {_LX1 = (position _UL) select 0;_LY1 = (position _UL) select 1;_halfway = true};
...
_BBProgN = RydHQE getVariable ["BBProgress",0];
...
if ((RydHQE_Debug) or (isPlayer (leader _unitG))) then {_i setMarkerColor "ColorBlue"};

_isTaken = false;
_waitTime = RydHQE_ObjHoldTime;
...
if (leaderHQE in (RydBBa_HQs + RydBBb_HQs)) then {_mGain = 0};
...
waitUntil
	{
	sleep 60;
	_BBProgN = RydHQE getVariable "BBProgress";
	if (isNil "_BBProgN") then {_BBProgN = 0};

	if ((RydHQE_NObj == _currentObj) and not (_BBProgN > _BBProg)) then
		{
		_waitTime = _waitTime - 60;

		_AllV0 = _trg nearEntities [["AllVehicles"],RydHQE_ObjRadius1];
		...
		_Civs0 = _trg nearEntities [["Civilian"],RydHQE_ObjRadius1];
		...
		_AllV20 = _trg nearEntities [["AllVehicles"],RydHQE_ObjRadius2];
		...
		if (not (RydHQE_UnlimitedCapt) and (_NearAllies >= RydHQE_CaptLimit) and (_NearEnemies <= (0 + ((RydHQE_Recklessness/(0.5 + RydHQE_Consistency))*10)))) then
			{
			RydHQE_NObj = RydHQE_NObj + 1;
			if ((random 100) < (15*RydHQE_NObj)) then
				{
				RydHQE_FlankingDone = false;
				RydHQE_FlankingInit = false
				};
			RydHQE_Morale = RydHQE_Morale + _mGain;
			_isTaken = true
			};
		...
		{
		if (not (RydHQE_NObj < _currentObj) or (RydBB_Active)) then
			{
			_isTaken = true
			}
		else
...
if ((RydHQE_UnlimitedCapt) or not (_isTaken)) then {_Trg setvariable [("Capturing" + str _Trg),[0,0]]};
...
_all = true;
	...
	{
	if ((_x in RydHQE_AttackAv) and not (_x getVariable ("Busy" + str (_x)))) exitwith {_all = false};
	}
foreach RydHQE_Friends - ((RydHQE_AirG - RydHQE_NCrewInfG) + RydHQE_NavalG + RydHQE_StaticG + RydHQE_SupportG + RydHQE_ArtG + (RydHQE_NCCargoG - (RydHQE_NCrewInfG - RydHQE_SupportG)));
