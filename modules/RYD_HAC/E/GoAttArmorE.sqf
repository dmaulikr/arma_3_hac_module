...
if ((RydHQE_Debug) or (isPlayer (leader _unitG))) then
	{
	_i = [[_posX,_posY],_unitG,"markAttack","ColorRed","ICON","mil_dot","Arm E"," - ATTACK"] call RYD_Mark;
	};
...
if ((RydHQE_Debug) or (isPlayer (leader _unitG))) then {deleteMarker _i};

_countAv = count RydHQE_AttackAv;
RydHQE_AttackAv = RydHQE_AttackAv + [(_unitG)];
_unitG setVariable [("Busy" + (str _unitG)),false];
