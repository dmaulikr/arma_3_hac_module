...
RydHQE_AmmoPoints = RydHQE_AmmoPoints + [_Trg];
...
if ((RydHQE_Debug) or (isPlayer (leader _unitG))) then
	{
	_i = [[_posX,_posY],_unitG,"markAmmoSupp","ColorKhaki","ICON","End"," Reammo E"," - AMMO SUPPORT",[0.6,0.6],270] call RYD_Mark
	};
...
if (_drop) then
	...
	if (_dst > 100) then
		...
		if (_unit isKindOf "Air") then
			...
			if not (_alive) exitwith {if ((RydHQE_Debug) or (isPlayer (leader _unitG))) then {deleteVehicle _ammoBox;deleteMarker ("markAmmoSupp" + str (_unitG))};RydHQE_AmmoPoints = RydHQE_AmmoPoints - [_Trg];_unitG setVariable [("Busy" + _unitvar), false];};
			if (_timer > 24) then
				{
				...
				RydHQE_Boxed set [(count RydHQE_Boxed),(group _Trg)];
				};
			...
			if not (_alive) exitwith {if ((RydHQE_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markAmmoSupp" + str (_unitG))};RydHQE_AmmoPoints = RydHQE_AmmoPoints - [_Trg];_unitG setVariable [("Busy" + _unitvar), false];};
			if (_timer > 24) then
				{
				[_unitG, (currentWaypoint _unitG)] setWaypointPosition [position (vehicle (leader _unitG)), 0]
				}
	...
	while {(_counter <= 3)} do
		...
		_tp = "MOVE";
		if (RydHQE_SupportWP) then {_tp = "SUPPORT"};
...
_tp = "MOVE";
if ((RydHQE_SupportWP) and not (_drop)) then {_tp = "SUPPORT"};
_pos = [_posX,_posY];
...
if ((damage _Trg) >= 0.9) then {_Hollow = _Hollow - [_Trg]};
if ((RydHQE_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markAmmoSupp" + str (_unitG))};
_lastOne = true;
...
if (_lastOne) then {RydHQE_ASupportedG = RydHQE_ASupportedG - [(group _Trg)]};
