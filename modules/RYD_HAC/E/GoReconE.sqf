...
RydHQE_ReconAv = RydHQE_ReconAv - [_unitG];

_UL = leader _unitG;
_PosLand = position _UL;
_nothing = true;
_End = [((position leaderHQE) select 0) + (random 400) - 200,((position leaderHQE) select 1) + (random 400) - 200];
_rd = 200;
...
if (_isWater) exitwith {RydHQE_ReconAv = RydHQE_ReconAv + [(_unitG)];_unitG setVariable [("Busy" + (str _unitG)),false]};
...
_eClose1 = [[_posX,_posY],RydHQE_KnEnemiesG,400] call RYD_CloseEnemyB;
...
if not (isNull _NeN) then
	{
	_eClose2 = [_UL,RydHQE_KnEnemiesG,400] call RYD_CloseEnemyB;
	_tooC2 = _eClose2 select 0;
	_dstEM2 = _eClose2 select 1;
	_eClose3 = [leaderHQE,RydHQE_KnEnemiesG,400] call RYD_CloseEnemyB;
...
if (not (isNull _GDV) and (_GDV in (RydHQE_NCCargoG + RydHQE_AirG)) and (_NeNMode)) then {_LX1 = (position _UL) select 0;_LY1 = (position _UL) select 1;_halfway = true};
...
_tp = "MOVE";
//if (not (isNull _AV) and (_unitG in RydHQE_NCrewInfG)) then {_tp = "UNLOAD"};
_beh = "SAFE";
if (not (isNull _AV) and (_GDV in RydHQE_AirG) or (_unitG in RydHQE_RAirG)) then {_beh = "CARELESS"};
_spd = "NORMAL";
...
_cause = [_unitG,6,true,600,50,[RydHQE_AirG,RydHQE_KnEnemiesG],false] call RYD_Wait;
...
if not (_alive) exitwith {if ((RydHQE_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markAttack" + str (_unitG))}};
...
_cause = [_unitG,6,true,250,30,[RydHQE_AirG,RydHQE_KnEnemiesG],false] call RYD_Wait;
_alive = _cause select 1;

if not (_alive) exitwith {if ((RydHQE_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markRecon" + str (_unitG))}};
if ((count (waypoints _unitG)) >= 1) then {deleteWaypoint [_unitG, 1]};

if (_stage >= 4) then {RydHQE_ReconDone = true};
...
if (_unitG in RydHQE_FOG) then
	...
	_cause = [_unitG,6,true,150,120,[RydHQE_AirG,RydHQE_KnEnemiesG],false] call RYD_Wait;
	_timer = _cause select 0;
	_alive = _cause select 1;
	_enemy = _cause select 2
	};
...
while {(_nothing)} do
	...
	if (((not (isNull (_UL findNearestEnemy _UL)) or (_timer2 > 4)) and not (isNull _unitG) and not (_unitG in RydHQE_FOG)) or ((_timer2 > 40) and not (isNull _unitG))) then
		...
		sleep 15;
		_rd = 0;
		if (_unitG in RydHQE_AirG) then {_End = _PosLand;_rd = 0} else {_End = [((position leaderHQE) select 0) + (random 400) - 200,((position leaderHQE) select 1) + (random 400) - 200];_isWater = surfaceIsWater _End;if (_isWater) then {_End = [((position leaderHQE) select 0) + (random 40) - 20,((position leaderHQE) select 1) + (random 40) - 20]}};
		...
		if ((RydHQE_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markRecon" + str (_unitG))};
		_nothing = false;
		...
		if (_alive) then
			{
			if (_unitG in (RydHQE_reconG + RydHQE_FOG + RydHQE_snipersG + RydHQE_InfG)) then {RydHQE_ReconAv = RydHQE_ReconAv + [_unitG];};
