...
_dX = (_PosObj1 select 0) - ((getPosATL leaderHQE) select 0);
_dY = (_PosObj1 select 1) - ((getPosATL leaderHQE) select 1);
...
if (_unitG in RydHQE_AirG) then {_distance2 = 800};
...
_posX = ((getPosATL leaderHQE) select 0) + _dXb + (random 200) - 100;
_posY = ((getPosATL leaderHQE) select 1) + _dYb + (random 200) - 100;
...
if not (_alive) exitwith {[_Trg,"SnpAttacked"] call RYD_VarReductor;if ((RydHQE_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markAttack" + str (_unitG))}};
