...
_Armor = RydHQE_LArmorG + RydHQE_HArmorG;

if (_unitG in _Armor) then {_distance2 = 500};
if (_unitG in RydHQE_AirG) then {_distance2 = 750};
...
if ((RydHQE_Debug) or (isPlayer (leader _unitG))) then
	{
	_i = [[_posX,_posY],_unitG,"markAttack","ColorRed","ICON","mil_dot","Inf E"," - ATTACK"] call RYD_Mark
	};
...
if ((isNull (leader (_this select 0))) or (_timer > 900)) exitwith {[_Trg,"InfAttacked"] call RYD_VarReductor;if ((RydHQE_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markAttack" + str (_unitG))};if not (isNull _GDV) then {[_GDV, (currentWaypoint _GDV)] setWaypointPosition [position (vehicle (leader _GDV)), 0];_GDV setVariable [("Busy" + _unitvar), false];}};
...
if (_unitG in RydHQE_Garrison) then
	{
	if (isPlayer (leader _unitG)) then
		{
		if not (isMultiplayer) then
...
_pass orderGetIn true;//_countAv = count RydHQE_AttackAv;
RydHQE_AttackAv = RydHQE_AttackAv + [(_unitG)];
_unitG setVariable [("Busy" + (str _unitG)),false];
