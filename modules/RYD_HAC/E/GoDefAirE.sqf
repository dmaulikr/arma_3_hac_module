...
if (_ammo == 0) exitwith {RydHQE_Roger = true};
...
if ((_busy) and (_unitG in RydHQE_AirInDef)) exitwith {RydHQE_Roger = true};
...
RydHQE_AirInDef = RydHQE_AirInDef + [_unitG];
...
while {not (_endThis)} do
	...
	if ((RydHQE_Debug) or (isPlayer (leader _unitG))) then
		{
		_i = [_DefPos,_unitG,"markDef","ColorBrown","ICON","mil_dot","Air E"," - DEFEND AREA"] call RYD_Mark
		};
	...
	if (_unitG in RydHQE_BAirG) then
		{
		_chosen = GrpNull;
		_dstM = 5000;

		_chosen = ([_Spot,RydHQE_KnEnemiesG,0] call RYD_CloseEnemyB) select 2;
...
if not (_alive) exitwith {if ((RydHQE_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markDef" + (str _unitG))};RydHQE_AirInDef = RydHQE_AirInDef - [_unitG]};

if ((isPlayer (leader _unitG)) and not (isMultiplayer)) then {(leader _unitG) removeSimpleTask _task};

sleep 30;

if ((RydHQE_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markDef" + (str _unitG))};
