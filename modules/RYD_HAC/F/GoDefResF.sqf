...
if ((RydHQF_Debug) or (isPlayer (leader _unitG))) then
	{
	_i = [_DefPos,_unitG,"markDef","ColorRed","ICON","DOT","DRes F"," - DEFEND POSITION"] call RYD_Mark
	};
...
if ((RydHQF_Debug) or (isPlayer (leader _unitG))) then
	{
	_i = [_TED,_unitG,"markWatch","ColorGreenAlpha","ICON","DOT","F","F",[0.2,0.2]] call RYD_Mark
	};
...
waituntil
	...
	if not (isNull _chosen) then
		...
		if ((RydHQF_Debug) or (isPlayer (leader _unitG))) then
			{
			_i = [_RnfP,_unitG,"markReinf","ColorRed","ICON","DOT","Reinf F"," - REINFORCE POSITION",[0.3,0.3]] call RYD_Mark
			}
		};
