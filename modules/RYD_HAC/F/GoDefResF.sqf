...
if ((RydHQF_Debug) or (isPlayer (leader _unitG))) then
	{
	_i = [_DefPos,_unitG,"markDef","ColorRedAlpha","ICON","mil_dot","DRes F"," - DEFEND POSITION"] call RYD_Mark
	};
...
waituntil
	...
	if not (isNull _chosen) then
		...
		if ((RydHQF_Debug) or (isPlayer (leader _unitG))) then
			{
			_i = [_RnfP,_unitG,"markReinf","ColorRedAlpha","ICON","mil_dot","Reinf F"," - REINFORCE POSITION",[0.3,0.3]] call RYD_Mark
			}
		};
