...
if (_ammo == 0) exitwith {RydHQF_Roger = true};
...
if ((_busy) and (_unitG in RydHQF_AirInDef)) exitwith {RydHQF_Roger = true};
...
RydHQF_AirInDef = RydHQF_AirInDef + [_unitG];
...
while {not (_endThis)} do
	...
	if ((RydHQF_Debug) or (isPlayer (leader _unitG))) then
		{
		_i = [_DefPos,_unitG,"markDef","ColorBrown","ICON","mil_dot","Air F"," - DEFEND AREA"] call RYD_Mark
		};
	...
	if (_unitG in RydHQF_BAirG) then
		...
		_chosen = ([_Spot,RydHQF_KnEnemiesG,0] call RYD_CloseEnemyB) select 2;
...
if not (_alive) exitwith {if ((RydHQF_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markDef" + (str _unitG))};RydHQF_AirInDef = RydHQF_AirInDef - [_unitG]};

if ((isPlayer (leader _unitG)) and not (isMultiplayer)) then {(leader _unitG) removeSimpleTask _task};

sleep 30;

if ((RydHQF_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markDef" + (str _unitG))};
