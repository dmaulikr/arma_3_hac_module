...
if (_unitG in RydHQF_Garrison) exitwith {RydHQF_DefSpot = RydHQF_DefSpot - [_unitG];RydHQF_GoodSpots = RydHQF_GoodSpots + [_DefPos];RydHQF_Roger = true};
...
//_dX = (((position leaderHQF) select 0) - (_Center select 0));
//_dY = (((position leaderHQF) select 1) - (_Center select 1));


if ((_unitG in (RydHQF_NCCargoG - RydHQF_AirG)) and ((count (units _unitG)) <= 1) or (_unitG in (RydHQF_SupportG - RydHQF_AirG))) then
	{
	_Xpos = ((position leaderHQF) select 0) + (random 300) - 150 - (_dX/1.25);
	_Ypos = ((position leaderHQF) select 1) + (random 300) - 150 - (_dY/1.25);
	_Defpos = [_Xpos,_Ypos];
	};
...
if ((RydHQF_Debug) or (isPlayer (leader _unitG))) then
	...
	_clr = "ColorBrown";
	_txt = "LMCU F";
	if (_unitG in (RydHQF_SupportG - RydHQF_AirG)) then {_clr = "ColorKhaki";_txt = "DSupp F"};
	_pltxt = " - DEFEND POSITION";
	if (_unitG in (RydHQF_SupportG - RydHQF_AirG)) then {_pltxt = " - SUPPORT"};
...
//if (_unitG in RydHQF_CargoG) then {(assignedCargo _AV) orderGetIn false};
...
if ((RydHQF_Debug) or (isPlayer (leader _unitG))) then
	{
	_i = [_TED,_unitG,"markWatch","ColorGreenAlpha","ICON","mil_dot","F","F",[0.2,0.2]] call RYD_Mark
	};
...
if not (_alive) exitWith
	...
	RydHQF_DefSpot = RydHQF_DefSpot - [_unitG];
