...
RydHQF_FuelPoints = RydHQF_FuelPoints + [_Trg];
...
if ((RydHQF_Debug) or (isPlayer (leader _unitG))) then
	{
	_i = [[_posX,_posY],_unitG,"markFuelSupp","ColorKhaki","ICON","End"," Refuel F"," - FUEL SUPPORT",[0.6,0.6]] call RYD_Mark
	};
...
while {(_counter <= 3)} do
	...
	_tp = "MOVE";
	if (RydHQF_SupportWP) then {_tp = "SUPPORT"};
	...
	if not (_alive) exitwith {if ((RydHQF_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markFuelSupp" + str (_unitG))};RydHQF_FuelPoints = RydHQF_FuelPoints - [_Trg];_unitG setVariable [("Busy" + _unitvar), false];};
...
if (((fuel _Trg) > 0.1) or ((damage _Trg) >= 0.9) or (isNull (group (assigneddriver (_this select 1))))) then {_dried = _dried - [_Trg]};
if ((RydHQF_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markFuelSupp" + str (_unitG))};
_lastOne = true;
...
if (_lastOne) then {RydHQF_FSupportedG = RydHQF_FSupportedG - [(group _Trg)]};
