	...
	if (_dst > 100) then
		...
		if (_unit isKindOf "Air") then
			...
			if (_timer > 24) then
				...
				RydHQF_Boxed set [(count RydHQF_Boxed),(group _Trg)];
				};
...
if ((damage _Trg) >= 0.9) then {_Hollow = _Hollow - [_Trg]};
if ((RydHQF_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markAmmoSupp" + str (_unitG))};
_lastOne = true;
...
if (_lastOne) then {RydHQF_ASupportedG = RydHQF_ASupportedG - [(group _Trg)]};
