...
_DefPos = _this select 1;if (_unitG in RydHQB_Garrison) exitwith {RydHQB_DefSpot = RydHQB_DefSpot - [_unitG];RydHQB_GoodSpots = RydHQB_GoodSpots + [_DefPos];RydHQB_Roger = true};
...
if ((_unitG in RydHQB_NCCargoG) and ((count (units _unitG)) <= 1)) then
	{
	_PosX = ((position leaderHQB) select 0) + (random 200) - 100;
	_PosY = ((position leaderHQB) select 1) + (random 200) - 100;
	_DefPos = [_posX,_posY]
	};
...
if ((RydHQB_Debug) or (isPlayer (leader _unitG))) then
	{
	_i = [_DefPos,_unitG,"markDef","ColorRedAlpha","ICON","mil_dot","DRes B"," - DEFEND POSITION"] call RYD_Mark
	};
...
_dX = 2000 * (sin RydHQB_Angle);
_dY = 2000 * (cos RydHQB_Angle);
...
if ((RydHQB_Debug) or (isPlayer (leader _unitG))) then
	{
	_i = [_TED,_unitG,"markWatch","ColorGreenAlpha","ICON","mil_dot","B","B",[0.2,0.2]] call RYD_Mark
	};
...
waituntil
	...
	if (_sumD > 0) then {_dstAv = _sumD/(count RydHQB_Friends)};
	...
	if ((count _tooClose) > 0) then
		{
		_HQinDanger = true;
		_HQsupporting = RydHQB getVariable "SPRTD";
		if (isNil "_HQsupporting") then {_HQsupporting = 0}
		};

	_HQchance = (80 - (_HQsupporting * 20))/(1 + (RydHQB_Recklessness * 0.5));
	...
	if ((_HQinDanger) and ((random 100) < _HQchance)) then
		...
		_unitG setVariable ["Reinforcing",RydHQB];
		RydHQB setVariable ["SPRTD",_HQsupporting + 1]
		}
	else
		{
		_ToReinf = RydHQB_Friends - (RydHQB_AirG + RydHQB_DefRes);
	...
	if not (isNull _chosen) then
		...
		if ((RydHQB_Debug) or (isPlayer (leader _unitG))) then
			{
			_i = [_RnfP,_unitG,"markReinf","ColorRedAlpha","ICON","mil_dot","Reinf B"," - REINFORCE POSITION",[0.3,0.3]] call RYD_Mark
			}
		};


	if not (_unitG getVariable "Defending") then {_endThis = true};
	if ((isNull _unitG) or (isNull RydHQB)) then {_endThis = true;_alive = false};
...
if ((RydHQB_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markDef" + (str _unitG));deleteMarker ("markWatch" + (str _unitG));deleteMarker ("markReinf" + (str _unitG))};
