...
if ((RydHQB_Debug) or (isPlayer (leader _unitG))) then
	{
	_i = [[_posX,_posY],_unitG,"markAttack","ColorRed","ICON","mil_dot","Arm B"," - ATTACK"] call RYD_Mark;
	};
...
if ((RydHQB_Debug) or (isPlayer (leader _unitG))) then {deleteMarker _i};

_countAv = count RydHQB_AttackAv;
RydHQB_AttackAv = RydHQB_AttackAv + [(_unitG)];
_unitG setVariable [("Busy" + (str _unitG)),false];
