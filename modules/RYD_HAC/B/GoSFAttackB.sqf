...
if (isNil ("RydHQB_Obj")) then {_default = position leaderHQB} else {_default = position RydHQB_Obj};
...
_max0Enemy = leaderHQB;
_min0Enemy = leaderHQB;

_max1Enemy = leaderHQB;
_min1Enemy = leaderHQB;

if not (isNil ("RydHQB_Obj")) then
	{
	_max0Enemy = RydHQB_Obj;
	_min0Enemy = RydHQB_Obj;

	_max1Enemy = RydHQB_Obj;
	_min1Enemy = RydHQB_Obj
	};

if not ((count RydHQB_KnEnemies) == 0) then
	{
	_max0Enemy = RydHQB_KnEnemies select _sel0Max;
	_min0Enemy = RydHQB_KnEnemies select _sel0Min;

	_max1Enemy = RydHQB_KnEnemies select _sel1Max;
	_min1Enemy = RydHQB_KnEnemies select _sel1Min
	};

_PosMid0 = (_Epos0Min + _Epos0Max)/2;
_PosMid1 = (_Epos1Min + _Epos1Max)/2;

_dX = (_PosMid0) - ((getPosATL leaderHQB) select 0);
_dY = (_Posmid1) - ((getPosATL leaderHQB) select 1);
...
_dX = _BEposX - ((getPosATL leaderHQB) select 0);
_dY = _BEposY - ((getPosATL leaderHQB) select 1);
...
_BorHQD = leaderHQB distance _BEnemyPos;
...
_posXWP1 = ((getPosATL leaderHQB) select 0) + _FlankPosX + _safeX1 + (random 200) - 100;
_posYWP1 = ((getPosATL leaderHQB) select 1) + _FlankPosY + _safeY1 + (random 200) - 100;
...
if ((leaderHQB distance [_posXWP1,_posYWP1]) > (leaderHQB distance [_posXWP2,_posYWP2])) then
	{
...
if ((_ammo > 0) and not (_busy)) then
	...
	RydHQB_SFTargets set [(count RydHQB_SFTargets),_trgG];
	...
	if ((RydHQB_Debug) or (isPlayer (leader _unitG))) then
		{
		_i1 = [[_posXWP1,_posYWP1],_unitG,"markSFFlank1","ColorOrange","ICON","mil_dot","SF1 B"," - SF FLANK 1"] call RYD_Mark;
		_i2 = [[_posXWP2,_posYWP2],_unitG,"markSFFlank2","ColorOrange","ICON","mil_dot","SF2 B"," - SF FLANK 2"] call RYD_Mark;
		_i3 = [[_posXWP3,_posYWP3],_unitG,"markSFFlank3","ColorOrange","ICON","mil_dot","SF3 B"," - SF FLANK 3"] call RYD_Mark;
		_i4 = [[_posXWP4,_posYWP4],_unitG,"markSFFlank4","ColorOrange","ICON","mil_dot","SF4 B"," - SF ATTACK"] call RYD_Mark
		};

	_CargoCheck = _unitG getvariable ("CC" + _unitvar);
	if (isNil ("_CargoCheck")) then {_unitG setVariable [("CC" + _unitvar), false]};
	_AV = assignedVehicle _UL;
	if ((RydHQB_CargoFind > 0) and (isNull _AV) and (([_posXWP4,_posYWP4] distance (vehicle _UL)) > 1000)) then {[_unitG] spawn B_SCargo } else {_unitG setVariable [("CC" + _unitvar), true]};
	if (RydHQB_CargoFind > 0) then
		{
	...
	if (not (isNull _AV) and (RydHQB_CargoFind > 0)) then
		{
		...
		if (RydHQB_LZ) then {deleteVehicle (_AV getVariable ["TempLZ",objNull])};
		_timer = _cause select 0
		};

	if ((isPlayer (leader _unitG)) and not (isMultiplayer)) then {(leader _unitG) removeSimpleTask _task};
	if ((isNull (leader (_this select 0))) or (_timer > 900)) exitwith {if ((RydHQB_Debug) or (isPlayer (leader _unitG))) then {{deleteMarker _x} foreach [_i1,_i2,_i3,_i4]};if not (isNull _GDV) then {[_GDV, (currentWaypoint _GDV)] setWaypointPosition [getPosATL (vehicle (leader _GDV)), 1];_GDV setVariable [("Busy" + _unitvar), false];}};
	...
	_beh = "AWARE";
	if (not (isNull _AV) and (_GDV in RydHQB_AirG)) then {_beh = "CARELESS"};
	_TO = [0,0,0];
	...
	if not (((group _DAV) == (group _UL)) or (isNull (group _DAV))) then
		...
		_cause = [_GDV,6,true,300,30,[RydHQB_AirG,RydHQB_KnEnemiesG],true] call RYD_Wait;
		_timer = _cause select 0;
		...
		{
		_cause = [_unitG,6,true,300,30,[RydHQB_AirG,RydHQB_KnEnemiesG],false] call RYD_Wait;
		_timer = _cause select 0;
		_alive = _cause select 1;
		_enemy = _cause select 2
		};
	...
	if (not (_alive) and not (_OtherGroup)) exitwith {if ((RydHQB_Debug) or (isPlayer (leader _unitG))) then {{deleteMarker _x} foreach [_i1,_i2,_i3,_i4]}};
	if (isNull (leader (_this select 0))) exitwith {if ((RydHQB_Debug) or (isPlayer (leader _unitG))) then {{deleteMarker _x} foreach [_i1,_i2,_i3,_i4]};if not (isNull _GDV) then {[_GDV, (currentWaypoint _GDV)] setWaypointPosition [getPosATL (vehicle (leader _GDV)), 1];_GDV setVariable [("Busy" + _unitvar), false];}};
	...
	if ((RydHQB_Debug) or (isPlayer (leader _unitG))) then {_i1 setMarkerColor "ColorBlue"};
	...
	if ((RydHQB_Debug) or (isPlayer (leader _unitG))) then {_i2 setMarkerColor "ColorBlue"};

	_tp = "MOVE";
	//if (not (isNull _AV) and (_unitG in RydHQB_NCrewInfG) and not ((_GDV == _unitG) or (_GDV in RydHQB_AirG))) then {_tp = "UNLOAD"};
	_grp = _unitG;
	if not (isNull _AV) then {_grp = _GDV};
	_beh = "AWARE";
	_lz = objNull;
	if (not (isNull _AV) and (_GDV in RydHQB_AirG)) then
		{
		_beh = "CARELESS";
		if (RydHQB_LZ) then
			{
	...
	//if (((group (assigneddriver _AV)) in RydHQB_AirG) and (_unitG in RydHQB_NCrewInfG)) then {_sts = ["true","(vehicle this) land 'GET OUT';deletewaypoint [(group this), 0]"]};
	_TO = [0,0,0];
	...
	if not (((group _DAV) == (group _UL)) or (isNull (group _DAV))) then
		...
		_cause = [_GDV,6,true,400,30,[RydHQB_AirG,RydHQB_KnEnemiesG],false] call RYD_Wait;
		_timer = _cause select 0;
		_alive = _cause select 1;
		_enemy = _cause select 2
		}
		...
		{
		_cause = [_unitG,6,true,400,30,[RydHQB_AirG,RydHQB_KnEnemiesG],false] call RYD_Wait;
		_timer = _cause select 0;
		_alive = _cause select 1;
		_enemy = _cause select 2
		};
	...
	if not ((_GDV == _unitG) or (isNull _GDV)) then
		...
		{
		if (_unitG in RydHQB_NCrewInfG) then {_pass orderGetIn false};
		};

	_DAV = assigneddriver _AV;
	_GDV = group _DAV;

	if (not (isNull _AV) and (RydHQB_CargoFind > 0) and (_unitG in RydHQB_NCrewInfG)) then
		{
	...
	if (RydHQB_LZ) then {deleteVehicle _lz};
	...
	if ((isNull (leader (_this select 0))) or (_timer > 240)) exitwith {if ((RydHQB_Debug) or (isPlayer (leader _unitG))) then {{deleteMarker _x} foreach [_i1,_i2,_i3,_i4]};if not (isNull _GDV) then {[_GDV, (currentWaypoint _GDV)] setWaypointPosition [getPosATL (vehicle (leader _GDV)), 1];_GDV setVariable [("Busy" + _unitvar), false];_pass orderGetIn true}};

	_unitvar = str _GDV;
	_timer = 0;
	if (not (isNull _GDV) and (_GDV in RydHQB_AirG) and not (isPlayer (leader _GDV))) then
		{
	...
	if ((RydHQB_Debug) or (isPlayer (leader _unitG))) then {_i3 setMarkerColor "ColorBlue"};
