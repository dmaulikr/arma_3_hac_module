...
_posX = ((getPos leaderHQB) select 0) + _dXb;
_posY = ((getPos leaderHQB) select 1) + _dYb;
...
if ((RydHQB_Debug) or (isPlayer (leader _unitG))) then
	{
	_i = [[_posX,_posY],_unitG,"markAttack","ColorRed","ICON","mil_dot","Air B"," - ATTACK"] call RYD_Mark
	};
...
if (_unitG in RydHQB_BAirG) then {deleteVehicle _lasT};
