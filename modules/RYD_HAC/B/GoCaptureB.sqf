...
if (isNil ("RydHQB_Obj")) then {_trg = leaderHQB} else {_Trg = RydHQB_Obj};
...
_currentObj = RydHQB_NObj;
_BBProg = RydHQB getVariable ["BBProgress",0];

_unitG setVariable [("Capt" + (str _unitG)),true];
RydHQB_AttackAv = RydHQB_AttackAv - [_unitG];

_UL = leader _unitG;
_nothing = true;

_dX = (_PosObj1 select 0) - ((getPos leaderHQB) select 0);
_dY = (_PosObj1 select 1) - ((getPos leaderHQB) select 1);

_angle = _dX atan2 _dY;

_distance = leaderHQB distance _PosObj1;
_distance2 = 100;
...
_posX = ((getPos leaderHQB) select 0) + _dXb + _dXc + (random 200) -  100;
_posY = ((getPos leaderHQB) select 1) + _dYb + _dYc + (random 200) -  100;
...
if ((RydHQB_Debug) or (isPlayer (leader _unitG))) then
	{
	_i = [[_posX,_posY],_unitG,"markCapture","ColorRed","ICON","mil_dot","Cap B"," - SECURE AREA"] call RYD_Mark
	};

_CargoCheck = _unitG getvariable ("CC" + _unitvar);
if (isNil ("_CargoCheck")) then {_unitG setVariable [("CC" + _unitvar), false]};
_AV = assignedVehicle _UL;
if ((RydHQB_CargoFind > 0) and (isNull _AV) and (([_posX,_posY] distance (vehicle _UL)) > 1000)) then {[_unitG] spawn B_SCargo } else {_unitG setVariable [("CC" + _unitvar), true]};
...
if ((isNull (leader (_this select 0))) or (_timer > 900)) exitwith
	...
	_isAttacked = _isAttacked - 1;
	_Trg setVariable [("Capturing" + (str _Trg)),[_isAttacked,_amountC]];
	if ((RydHQB_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markCapture" + str (_unitG))};
...
_eClose1 = [[_posX,_posY],RydHQB_KnEnemiesG,300] call RYD_CloseEnemyB;
...
if not (isNull _NeN) then
	{
	_eClose2 = [_UL,RydHQB_KnEnemiesG,300] call RYD_CloseEnemyB;
	_tooC2 = _eClose2 select 0;
	_dstEM2 = _eClose2 select 1;
	_eClose3 = [leaderHQB,RydHQB_KnEnemiesG,300] call RYD_CloseEnemyB;
...
if (not (isNull _GDV) and (_GDV in (RydHQB_NCCargoG + RydHQB_AirG)) and (_NeNMode) and not (isPlayer (leader _GDV))) then {_LX1 = (position _UL) select 0;_LY1 = (position _UL) select 1;_halfway = true};
...
_BBProgN = RydHQB getVariable ["BBProgress",0];
...
if ((RydHQB_Debug) or (isPlayer (leader _unitG))) then {_i setMarkerColor "ColorBlue"};

_isTaken = false;
_waitTime = RydHQB_ObjHoldTime;
...
if (leaderHQB in (RydBBa_HQs + RydBBb_HQs)) then {_mGain = 0};
...
waitUntil
	{
	sleep 60;
	_BBProgN = RydHQB getVariable "BBProgress";
	if (isNil "_BBProgN") then {_BBProgN = 0};

	if ((RydHQB_NObj == _currentObj) and not (_BBProgN > _BBProg)) then
		{
		_waitTime = _waitTime - 60;

		_AllV0 = _trg nearEntities [["AllVehicles"],RydHQB_ObjRadius1];
		...
		_Civs0 = _trg nearEntities [["Civilian"],RydHQB_ObjRadius1];
		...
		_AllV20 = _trg nearEntities [["AllVehicles"],RydHQB_ObjRadius2];
		...
		if (not (RydHQB_UnlimitedCapt) and (_NearAllies >= RydHQB_CaptLimit) and (_NearEnemies <= (0 + ((RydHQB_Recklessness/(0.5 + RydHQB_Consistency))*10)))) then
			{
			RydHQB_NObj = RydHQB_NObj + 1;
			if ((random 100) < (15*RydHQB_NObj)) then
				{
				RydHQB_FlankingDone = false;
				RydHQB_FlankingInit = false
				};
			RydHQB_Morale = RydHQB_Morale + _mGain;
			_isTaken = true
			};
		...
		{
		if (not (RydHQB_NObj < _currentObj) or (RydBB_Active)) then
			{
			_isTaken = true
			}
		else
...
if ((RydHQB_UnlimitedCapt) or not (_isTaken)) then {_Trg setvariable [("Capturing" + str _Trg),[0,0]]};
...
_all = true;
	...
	{
	if ((_x in RydHQB_AttackAv) and not (_x getVariable ("Busy" + str (_x)))) exitwith {_all = false};
	}
foreach RydHQB_Friends - ((RydHQB_AirG - RydHQB_NCrewInfG) + RydHQB_NavalG + RydHQB_StaticG + RydHQB_SupportG + RydHQB_ArtG + (RydHQB_NCCargoG - (RydHQB_NCrewInfG - RydHQB_SupportG)));
