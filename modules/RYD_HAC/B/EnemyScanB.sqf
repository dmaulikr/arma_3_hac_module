...
if (RydHQB_DebugII) then
	{
	waitUntil
		{
		sleep 1;
		not (isNil "RydHQB_KnEnemiesG")
		};
...
while {not (isNull RydHQB)} do
	{
	_LCU = RydHQB_Friends - (RydHQB_NavalG + RydHQB_SupportG + RydHQB_ArtG + (RydHQB_NCCargoG - (RydHQB_NCrewInfG - RydHQB_SupportG)));
	...
	foreach (RydHQB_Friends - RydHQB_AirG);
