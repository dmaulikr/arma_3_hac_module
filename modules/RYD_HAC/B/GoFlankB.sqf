...
if (_unitG in RydHQB_Garrison) exitwith {RydHQB_FlankAv = RydHQB_FlankAv - [_unitG]};
...
if (_isWater) exitwith {_unitG setVariable [("Busy" + (str _unitG)),false];RydHQB_FlankAv = RydHQB_FlankAv - [_unitG]};
...
if ((_ammo > 0) and not (_busy)) then
	...
	if ((RydHQB_Debug) or (isPlayer (leader _unitG))) then
		{
		_i1 = [[_posXWP1,_posYWP1],_unitG,"markFlank1","ColorOrange","ICON","mil_dot","Fl1 B"," - FLANK 1"] call RYD_Mark;
		_i2 = [[_posXWP2,_posYWP2],_unitG,"markFlank2","ColorOrange","ICON","mil_dot","Fl2 B"," - FLANK 2"] call RYD_Mark;
		_i3 = [[_posXWP3,_posYWP3],_unitG,"markFlank3","ColorOrange","ICON","mil_dot","Fl3 B"," - FLANK 3"] call RYD_Mark;
		_i4 = [[_posXWP4,_posYWP4],_unitG,"markFlank4","ColorOrange","ICON","mil_dot","Fl4 B"," - FLANKING ATTACK"] call RYD_Mark
		};
	...
	if ((isPlayer (leader _unitG)) and not (isMultiplayer)) then {(leader _unitG) removeSimpleTask _task};
	RydHQB_FlankAv = RydHQB_FlankAv - [_unitG];
	_unitG setVariable [("Busy" + _unitvar), false];

	if ((RydHQB_Debug) or (isPlayer (leader _unitG))) then {_i4 setMarkerColor "ColorBlue"};
	sleep 30;
	if ((RydHQB_Debug) or (isPlayer (leader _unitG))) then
		{
			{
