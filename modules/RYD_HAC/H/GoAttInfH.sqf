...
_Armor = RydHQH_LArmorG + RydHQH_HArmorG;

if (_unitG in _Armor) then {_distance2 = 500};
if (_unitG in RydHQH_AirG) then {_distance2 = 750};
...
if ((RydHQH_Debug) or (isPlayer (leader _unitG))) then
	{
	_i = [[_posX,_posY],_unitG,"markAttack","ColorRed","ICON","mil_dot","Inf H"," - ATTACK"] call RYD_Mark
	};
...
if ((isNull (leader (_this select 0))) or (_timer > 900)) exitwith {[_Trg,"InfAttacked"] call RYD_VarReductor;if ((RydHQH_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markAttack" + str (_unitG))};if not (isNull _GDV) then {[_GDV, (currentWaypoint _GDV)] setWaypointPosition [position (vehicle (leader _GDV)), 0];_GDV setVariable [("Busy" + _unitvar), false];}};
...
_pass orderGetIn true;//_countAv = count RydHQH_AttackAv;
RydHQH_AttackAv = RydHQH_AttackAv + [(_unitG)];
_unitG setVariable [("Busy" + (str _unitG)),false];
