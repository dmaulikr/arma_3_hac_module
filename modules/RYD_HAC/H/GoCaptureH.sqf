...
if (isNil ("RydHQH_Obj")) then {_trg = leaderHQH} else {_Trg = RydHQH_Obj};
...
_currentObj = RydHQH_NObj;
_BBProg = RydHQH getVariable ["BBProgress",0];
...
if ((RydHQH_Debug) or (isPlayer (leader _unitG))) then
	{
	_i = [[_posX,_posY],_unitG,"markCapture","ColorRed","ICON","mil_dot","Cap H"," - SECURE AREA"] call RYD_Mark
	};

_CargoCheck = _unitG getvariable ("CC" + _unitvar);
if (isNil ("_CargoCheck")) then {_unitG setVariable [("CC" + _unitvar), false]};
_AV = assignedVehicle _UL;
if ((RydHQH_CargoFind > 0) and (isNull _AV) and (([_posX,_posY] distance (vehicle _UL)) > 1000)) then {[_unitG] spawn H_SCargo } else {_unitG setVariable [("CC" + _unitvar), true]};
...
if ((isNull (leader (_this select 0))) or (_timer > 900)) exitwith
	...
	_isAttacked = _isAttacked - 1;
	_Trg setVariable [("Capturing" + (str _Trg)),[_isAttacked,_amountC]];
	if ((RydHQH_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markCapture" + str (_unitG))};
...
_eClose1 = [[_posX,_posY],RydHQH_KnEnemiesG,300] call RYD_CloseEnemyB;
...
if not (isNull _NeN) then
	{
	_eClose2 = [_UL,RydHQH_KnEnemiesG,300] call RYD_CloseEnemyB;
	_tooC2 = _eClose2 select 0;
	_dstEM2 = _eClose2 select 1;
	_eClose3 = [leaderHQH,RydHQH_KnEnemiesG,300] call RYD_CloseEnemyB;
...
if (not (isNull _GDV) and (_GDV in (RydHQH_NCCargoG + RydHQH_AirG)) and (_NeNMode) and not (isPlayer (leader _GDV))) then {_LX1 = (position _UL) select 0;_LY1 = (position _UL) select 1;_halfway = true};
...
_BBProgN = RydHQH getVariable ["BBProgress",0];
...
if (leaderHQH in (RydBBa_HQs + RydBBb_HQs)) then {_mGain = 0};
...
waitUntil
	{
	sleep 60;
	_BBProgN = RydHQH getVariable "BBProgress";
	if (isNil "_BBProgN") then {_BBProgN = 0};

	if ((RydHQH_NObj == _currentObj) and not (_BBProgN > _BBProg)) then
		{
		_waitTime = _waitTime - 60;

		_AllV0 = _trg nearEntities [["AllVehicles"],RydHQH_ObjRadius1];
		...
		_Civs0 = _trg nearEntities [["Civilian"],RydHQH_ObjRadius1];
		...
		_AllV20 = _trg nearEntities [["AllVehicles"],RydHQH_ObjRadius2];
		...
		if (not (RydHQH_UnlimitedCapt) and (_NearAllies >= RydHQH_CaptLimit) and (_NearEnemies <= (0 + ((RydHQH_Recklessness/(0.5 + RydHQH_Consistency))*10)))) then
			{
			RydHQH_NObj = RydHQH_NObj + 1;
			if ((random 100) < (15*RydHQH_NObj)) then
				{
				RydHQH_FlankingDone = false;
				RydHQH_FlankingInit = false
				};
			RydHQH_Morale = RydHQH_Morale + _mGain;
			_isTaken = true
			};
		...
		{
		if (not (RydHQH_NObj < _currentObj) or (RydBB_Active)) then
			{
			_isTaken = true
			}
		else
...
if ((RydHQH_UnlimitedCapt) or not (_isTaken)) then {_Trg setvariable [("Capturing" + str _Trg),[0,0]]};
...
_all = true;
	...
	{
	if ((_x in RydHQH_AttackAv) and not (_x getVariable ("Busy" + str (_x)))) exitwith {_all = false};
	}
foreach RydHQH_Friends - ((RydHQH_AirG - RydHQH_NCrewInfG) + RydHQH_NavalG + RydHQH_StaticG + RydHQH_SupportG + RydHQH_ArtG + (RydHQH_NCCargoG - (RydHQH_NCrewInfG - RydHQH_SupportG)));
