...
if (RydHQH_DebugII) then
	{
	waitUntil
		{
		sleep 1;
		not (isNil "RydHQH_KnEnemiesG")
		};
...
while {not (isNull RydHQH)} do
	{
	_LCU = RydHQH_Friends - (RydHQH_NavalG + RydHQH_SupportG + RydHQH_ArtG + (RydHQH_NCCargoG - (RydHQH_NCrewInfG - RydHQH_SupportG)));
	...
	foreach (RydHQH_Friends - RydHQH_AirG);
