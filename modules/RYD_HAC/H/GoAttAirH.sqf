...
_posX = ((getPos leaderHQH) select 0) + _dXb;
_posY = ((getPos leaderHQH) select 1) + _dYb;
...
if ((RydHQH_Debug) or (isPlayer (leader _unitG))) then
	{
	_i = [[_posX,_posY],_unitG,"markAttack","ColorRed","ICON","mil_dot","Air H"," - ATTACK"] call RYD_Mark
	};
...
if (_unitG in RydHQH_BAirG) then {deleteVehicle _lasT};
