...
if (_unitG in RydHQH_Garrison) exitwith {RydHQH_FlankAv = RydHQH_FlankAv - [_unitG]};
...
_dX = _BEposX - ((getPosATL leaderHQH) select 0);
_dY = _BEposY - ((getPosATL leaderHQH) select 1);
...
_BorHQD = leaderHQH distance _BEnemyPos;
...
_posXWP1 = ((getPosATL leaderHQH) select 0) + _FlankPosX + _safeX1 + (random 200) - 100;
_posYWP1 = ((getPosATL leaderHQH) select 1) + _FlankPosY + _safeY1 + (random 200) - 100;
...
if (_isWater) exitwith {_unitG setVariable [("Busy" + (str _unitG)),false];RydHQH_FlankAv = RydHQH_FlankAv - [_unitG]};
...
if ((_ammo > 0) and not (_busy)) then
	...
	if ((RydHQH_Debug) or (isPlayer (leader _unitG))) then
		{
		_i1 = [[_posXWP1,_posYWP1],_unitG,"markFlank1","ColorOrange","ICON","mil_dot","Fl1 H"," - FLANK 1"] call RYD_Mark;
		_i2 = [[_posXWP2,_posYWP2],_unitG,"markFlank2","ColorOrange","ICON","mil_dot","Fl2 H"," - FLANK 2"] call RYD_Mark;
		_i3 = [[_posXWP3,_posYWP3],_unitG,"markFlank3","ColorOrange","ICON","mil_dot","Fl3 H"," - FLANK 3"] call RYD_Mark;
		_i4 = [[_posXWP4,_posYWP4],_unitG,"markFlank4","ColorOrange","ICON","mil_dot","Fl4 H"," - FLANKING ATTACK"] call RYD_Mark
		};
	...
	if (not (isNull _AV) and (RydHQH_CargoFind > 0)) then
		{
		...
		if (RydHQH_LZ) then {deleteVehicle (_AV getVariable ["TempLZ",objNull])};
		_timer = _cause select 0
		};

	if ((isPlayer (leader _unitG)) and not (isMultiplayer)) then {(leader _unitG) removeSimpleTask _task};
	if ((isNull (leader (_this select 0))) or (_timer > 900)) exitwith {if ((RydHQH_Debug) or (isPlayer (leader _unitG))) then {{deleteMarker _x} foreach [_i1,_i2,_i3,_i4]};if not (isNull _GDV) then {[_GDV, (currentWaypoint _GDV)] setWaypointPosition [getPosATL (vehicle (leader _GDV)), 1];_GDV setVariable [("Busy" + _unitvar), false];}};
	...
	_beh = "SAFE";
	if (not (isNull _AV) and (_GDV in RydHQH_AirG)) then {_beh = "CARELESS"};
	_TO = [0,0,0];
	...
	if not (((group _DAV) == (group _UL)) or (isNull (group _DAV))) then
		...
		_cause = [_GDV,6,true,300,30,[RydHQH_AirG,RydHQH_KnEnemiesG],true] call RYD_Wait;
		_timer = _cause select 0;
	...
	if (not (_alive) and not (_OtherGroup)) exitwith {if ((RydHQH_Debug) or (isPlayer (leader _unitG))) then {{deleteMarker _x} foreach [_i1,_i2,_i3,_i4]}};
	if (isNull (leader (_this select 0))) exitwith {if ((RydHQH_Debug) or (isPlayer (leader _unitG))) then {{deleteMarker _x} foreach [_i1,_i2,_i3,_i4]};if not (isNull _GDV) then {[_GDV, (currentWaypoint _GDV)] setWaypointPosition [getPosATL (vehicle (leader _GDV)), 1];_GDV setVariable [("Busy" + _unitvar), false];}};
	...
	if ((RydHQH_Debug) or (isPlayer (leader _unitG))) then {_i1 setMarkerColor "ColorBlue"};
	...
	if ((RydHQH_Debug) or (isPlayer (leader _unitG))) then {_i2 setMarkerColor "ColorBlue"};

	_tp = "MOVE";
	//if (not (isNull _AV) and (_unitG in RydHQH_NCrewInfG) and not ((_GDV == _unitG) or (_GDV in RydHQH_AirG))) then {_tp = "UNLOAD"};
	_grp = _unitG;
	if not (isNull _AV) then {_grp = _GDV};
	_beh = "AWARE";
	_lz = objNull;
	...
	if not (((group _DAV) == (group _UL)) or (isNull (group _DAV))) then
		...
		_cause = [_GDV,6,true,400,30,[RydHQH_AirG,RydHQH_KnEnemiesG],false] call RYD_Wait;
		_timer = _cause select 0;
		_alive = _cause select 1;
		_enemy = _cause select 2
		}
	...
	if ((isNull (leader (_this select 0))) or (_timer > 240)) exitwith {if ((RydHQH_Debug) or (isPlayer (leader _unitG))) then {{deleteMarker _x} foreach [_i1,_i2,_i3,_i4]};if not (isNull _GDV) then {[_GDV, (currentWaypoint _GDV)] setWaypointPosition [getPosATL (vehicle (leader _GDV)), 1];_GDV setVariable [("Busy" + _unitvar), false];_pass orderGetIn true}};

	_unitvar = str _GDV;
	_timer = 0;
	if (not (isNull _GDV) and (_GDV in RydHQH_AirG) and not (isPlayer (leader _GDV))) then
		{
	...
	if ((isPlayer (leader _unitG)) and not (isMultiplayer)) then {(leader _unitG) removeSimpleTask _task};
	RydHQH_FlankAv = RydHQH_FlankAv - [_unitG];
	_unitG setVariable [("Busy" + _unitvar), false];

	if ((RydHQH_Debug) or (isPlayer (leader _unitG))) then {_i4 setMarkerColor "ColorBlue"};
	sleep 30;
	if ((RydHQH_Debug) or (isPlayer (leader _unitG))) then
		{
			{
