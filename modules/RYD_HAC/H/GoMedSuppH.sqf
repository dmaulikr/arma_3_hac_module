...
RydHQH_MedPoints = RydHQH_MedPoints + [_Trg];
...
if ((RydHQH_Debug) or (isPlayer (leader _unitG))) then
	{
	_i = [[_posX,_posY],_unitG,"markMedSupp","ColorKhaki","ICON","Destroy"," Medic H"," - MEDICAL SUPPORT",[0.6,0.6]] call RYD_Mark
	};
...
while {(_counter <= 3)} do
	...
	if not (_alive) exitwith {if ((RydHQH_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markMedSupp" + str (_unitG))};RydHQH_MedPoints = RydHQH_MedPoints - [_Trg];_unitG setVariable [("Busy" + _unitvar), false];};
...
if (((damage _Trg) < 0.1) or ((damage _Trg) >= 0.9) or (isNull (group (_this select 1)))) then {_wounded = _wounded - [_Trg]};
if ((RydHQH_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markMedSupp" + str (_unitG))};
_lastOne = true;
...
if (_lastOne) then {RydHQH_SupportedG = RydHQH_SupportedG - [(group _Trg)]};
