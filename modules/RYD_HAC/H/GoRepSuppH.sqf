...
RydHQH_RepPoints = RydHQH_RepPoints + [_Trg];
...
if ((RydHQH_Debug) or (isPlayer (leader _unitG))) then
	{
	_i = [[_posX,_posY],_unitG,"markRepSupp","ColorKhaki","ICON","End"," Repair H"," - REPAIR SUPPORT",[0.6,0.6],180] call RYD_Mark
	};
...
while {(_counter <= 3)} do
	...
	if not (_alive) exitwith {if ((RydHQH_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markRepSupp" + str (_unitG))};RydHQH_RepPoints = RydHQH_RepPoints - [_Trg];_unitG setVariable [("Busy" + _unitvar), false];};
...
if (((damage _Trg) < 0.1) or ((damage _Trg) >= 0.9) or (isNull (group (assigneddriver (_this select 1))))) then {_damaged = _damaged - [_Trg]};
if ((RydHQH_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markRepSupp" + str (_unitG))};
_lastOne = true;
...
if (_lastOne) then {RydHQH_RSupportedG = RydHQH_RSupportedG - [(group _Trg)]};
