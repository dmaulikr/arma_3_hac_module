...
if (_unitG in RydHQ_Garrison) exitwith {RydHQ_FlankAv = RydHQ_FlankAv - [_unitG]};
...
_dX = _BEposX - ((getPosATL leaderHQ) select 0);
_dY = _BEposY - ((getPosATL leaderHQ) select 1);
...
_posXWP1 = ((getPosATL leaderHQ) select 0) + _FlankPosX + _safeX1 + (random 200) - 100;
_posYWP1 = ((getPosATL leaderHQ) select 1) + _FlankPosY + _safeY1 + (random 200) - 100;
...
if (_isWater) exitwith {_unitG setVariable [("Busy" + (str _unitG)),false];RydHQ_FlankAv = RydHQ_FlankAv - [_unitG]};
...
if ((_ammo > 0) and not (_busy)) then
	...
	if ((RydHQ_Debug) or (isPlayer (leader _unitG))) then
		{
		_i1 = [[_posXWP1,_posYWP1],_unitG,"markFlank1","ColorOrange","ICON","mil_dot","Fl1 A"," - FLANK 1"] call RYD_Mark;
		_i2 = [[_posXWP2,_posYWP2],_unitG,"markFlank2","ColorOrange","ICON","mil_dot","Fl2 A"," - FLANK 2"] call RYD_Mark;
		_i3 = [[_posXWP3,_posYWP3],_unitG,"markFlank3","ColorOrange","ICON","mil_dot","Fl3 A"," - FLANK 3"] call RYD_Mark;
		_i4 = [[_posXWP4,_posYWP4],_unitG,"markFlank4","ColorOrange","ICON","mil_dot","Fl4 A"," - FLANKING ATTACK"] call RYD_Mark
		};

	_CargoCheck = _unitG getvariable ("CC" + _unitvar);
	if (isNil ("_CargoCheck")) then {_unitG setVariable [("CC" + _unitvar), false]};
	_AV = assignedVehicle _UL;
	if ((RydHQ_CargoFind > 0) and (isNull _AV) and (([_posXWP4,_posYWP4] distance (vehicle _UL)) > 1000)) then {[_unitG] spawn A_SCargo } else {_unitG setVariable [("CC" + _unitvar), true]};
	...
	if (not (isNull _AV) and (RydHQ_CargoFind > 0)) then
		{
		...
		if (RydHQ_LZ) then {deleteVehicle (_AV getVariable ["TempLZ",objNull])};
		_timer = _cause select 0
		};

	if ((isPlayer (leader _unitG)) and not (isMultiplayer)) then {(leader _unitG) removeSimpleTask _task};
	if ((isNull (leader (_this select 0))) or (_timer > 900)) exitwith {if ((RydHQ_Debug) or (isPlayer (leader _unitG))) then {{deleteMarker _x} foreach [_i1,_i2,_i3,_i4]};if not (isNull _GDV) then {[_GDV, (currentWaypoint _GDV)] setWaypointPosition [getPosATL (vehicle (leader _GDV)), 1];_GDV setVariable [("Busy" + _unitvar), false];}};
	...
	_beh = "SAFE";
	if (not (isNull _AV) and (_GDV in RydHQ_AirG)) then {_beh = "CARELESS"};
	_TO = [0,0,0];
	...
	if not (((group _DAV) == (group _UL)) or (isNull (group _DAV))) then
		...
		_cause = [_GDV,6,true,300,30,[RydHQ_AirG,RydHQ_KnEnemiesG],true] call RYD_Wait;
		_timer = _cause select 0;
		...
		{
		_cause = [_unitG,6,true,300,30,[RydHQ_AirG,RydHQ_KnEnemiesG],false] call RYD_Wait;
		_timer = _cause select 0;
		_alive = _cause select 1;
		_enemy = _cause select 2
		};
	...
	if (not (_alive) and not (_OtherGroup)) exitwith {if ((RydHQ_Debug) or (isPlayer (leader _unitG))) then {{deleteMarker _x} foreach [_i1,_i2,_i3,_i4]}};
	if (isNull (leader (_this select 0))) exitwith {if ((RydHQ_Debug) or (isPlayer (leader _unitG))) then {{deleteMarker _x} foreach [_i1,_i2,_i3,_i4]};if not (isNull _GDV) then {[_GDV, (currentWaypoint _GDV)] setWaypointPosition [getPosATL (vehicle (leader _GDV)), 1];_GDV setVariable [("Busy" + _unitvar), false];}};
	...
	if ((RydHQ_Debug) or (isPlayer (leader _unitG))) then {_i1 setMarkerColor "ColorBlue"};
	...
	if ((RydHQ_Debug) or (isPlayer (leader _unitG))) then {_i2 setMarkerColor "ColorBlue"};

	_tp = "MOVE";
	//if (not (isNull _AV) and (_unitG in RydHQ_NCrewInfG) and not ((_GDV == _unitG) or (_GDV in RydHQ_AirG))) then {_tp = "UNLOAD"};
	_grp = _unitG;
	if not (isNull _AV) then {_grp = _GDV};
	_beh = "AWARE";
	_lz = objNull;
	if (not (isNull _AV) and (_GDV in RydHQ_AirG)) then
		{
		_beh = "CARELESS";
		if (RydHQ_LZ) then
			{
	...
	if not (((group _DAV) == (group _UL)) or (isNull (group _DAV))) then
		...
		_cause = [_GDV,6,true,400,30,[RydHQ_AirG,RydHQ_KnEnemiesG],false] call RYD_Wait;
		_timer = _cause select 0;
		_alive = _cause select 1;
		_enemy = _cause select 2
		}
		...
		{
		_cause = [_unitG,6,true,400,30,[RydHQ_AirG,RydHQ_KnEnemiesG],false] call RYD_Wait;
		_timer = _cause select 0;
		_alive = _cause select 1;
		_enemy = _cause select 2
		};
	...
	if not ((_GDV == _unitG) or (isNull _GDV)) then
		...
		{
		if (_unitG in RydHQ_NCrewInfG) then {_pass orderGetIn false};
		};

	_DAV = assigneddriver _AV;
	_GDV = group _DAV;

	if (not (isNull _AV) and (RydHQ_CargoFind > 0) and (_unitG in RydHQ_NCrewInfG)) then
		{
	...
	if (RydHQ_LZ) then {deleteVehicle _lz};
	...
	if ((isNull (leader (_this select 0))) or (_timer > 240)) exitwith {if ((RydHQ_Debug) or (isPlayer (leader _unitG))) then {{deleteMarker _x} foreach [_i1,_i2,_i3,_i4]};if not (isNull _GDV) then {[_GDV, (currentWaypoint _GDV)] setWaypointPosition [getPosATL (vehicle (leader _GDV)), 1];_GDV setVariable [("Busy" + _unitvar), false];_pass orderGetIn true}};

	_unitvar = str _GDV;
	_timer = 0;
	if (not (isNull _GDV) and (_GDV in RydHQ_AirG) and not (isPlayer (leader _GDV))) then
		{
	...
	if ((RydHQ_Debug) or (isPlayer (leader _unitG))) then {_i3 setMarkerColor "ColorBlue"};
	...
	if ((RydHQ_Debug) or (isPlayer (leader _unitG))) then {_i4 setMarkerColor "ColorBlue"};
	sleep 30;
	if ((RydHQ_Debug) or (isPlayer (leader _unitG))) then
		{
			{
