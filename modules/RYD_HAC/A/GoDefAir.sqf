...
while {not (_endThis)} do
	...
	if ((RydHQ_Debug) or (isPlayer (leader _unitG))) then
		{
		_i = [_DefPos,_unitG,"markDef","ColorBrown","ICON","mil_dot","Air A"," - DEFEND AREA"] call RYD_Mark
		};
	...
	if (_unitG in RydHQ_BAirG) then
		{
		_chosen = GrpNull;
		_dstM = 5000;

		_chosen = ([_Spot,RydHQ_KnEnemiesG,0] call RYD_CloseEnemyB) select 2;
...
if not (_alive) exitwith {if ((RydHQ_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markDef" + (str _unitG))};RydHQ_AirInDef = RydHQ_AirInDef - [_unitG]};

if ((isPlayer (leader _unitG)) and not (isMultiplayer)) then {(leader _unitG) removeSimpleTask _task};

sleep 30;

if ((RydHQ_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markDef" + (str _unitG))};
