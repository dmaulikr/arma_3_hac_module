...
_currentObj = RydHQ_NObj;
_BBProg = RydHQ getVariable ["BBProgress",0];
...
if ((isNull (leader (_this select 0))) or (_timer > 900)) exitwith
	...
	_isAttacked = _isAttacked - 1;
	_Trg setVariable [("Capturing" + (str _Trg)),[_isAttacked,_amountC]];
	if ((RydHQ_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markCapture" + str (_unitG))};
...
_eClose1 = [[_posX,_posY],RydHQ_KnEnemiesG,300] call RYD_CloseEnemyB;
...
if not (isNull _NeN) then
	{
	_eClose2 = [_UL,RydHQ_KnEnemiesG,300] call RYD_CloseEnemyB;
	_tooC2 = _eClose2 select 0;
	_dstEM2 = _eClose2 select 1;
	_eClose3 = [leaderHQ,RydHQ_KnEnemiesG,300] call RYD_CloseEnemyB;
...
if (not (isNull _GDV) and (_GDV in (RydHQ_NCCargoG + RydHQ_AirG)) and (_NeNMode) and not (isPlayer (leader _GDV))) then {_LX1 = (position _UL) select 0;_LY1 = (position _UL) select 1;_halfway = true};
...
_BBProgN = RydHQ getVariable ["BBProgress",0];
...
if (leaderHQ in (RydBBa_HQs + RydBBb_HQs)) then {_mGain = 0};
...
waitUntil
	{
	sleep 60;
	_BBProgN = RydHQ getVariable "BBProgress";
	if (isNil "_BBProgN") then {_BBProgN = 0};

	if ((RydHQ_NObj == _currentObj) and not (_BBProgN > _BBProg)) then
		{
		_waitTime = _waitTime - 60;

		_AllV0 = _trg nearEntities [["AllVehicles"],RydHQ_ObjRadius1];
		...
		_Civs0 = _trg nearEntities [["Civilian"],RydHQ_ObjRadius1];
		...
		_AllV20 = _trg nearEntities [["AllVehicles"],RydHQ_ObjRadius2];
		...
		if (not (RydHQ_UnlimitedCapt) and (_NearAllies >= RydHQ_CaptLimit) and (_NearEnemies <= (0 + ((RydHQ_Recklessness/(0.5 + RydHQ_Consistency))*10)))) then
			{
			RydHQ_NObj = RydHQ_NObj + 1;
			if ((random 100) < (15*RydHQ_NObj)) then
				{
				RydHQ_FlankingDone = false;
				RydHQ_FlankingInit = false
				};
			RydHQ_Morale = RydHQ_Morale + _mGain;
			_isTaken = true
			};
		...
		{
		if (not (RydHQ_NObj < _currentObj) or (RydBB_Active)) then
			{
			_isTaken = true
			}
		else
...
if ((RydHQ_UnlimitedCapt) or not (_isTaken)) then {_Trg setvariable [("Capturing" + str _Trg),[0,0]]};
...
foreach RydHQ_Friends - ((RydHQ_AirG - RydHQ_NCrewInfG) + RydHQ_NavalG + RydHQ_StaticG + RydHQ_SupportG + RydHQ_ArtG + (RydHQ_NCCargoG - (RydHQ_NCrewInfG - RydHQ_SupportG)));
