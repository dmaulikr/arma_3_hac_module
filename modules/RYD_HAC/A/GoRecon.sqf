...
RydHQ_ReconAv = RydHQ_ReconAv - [_unitG];

_UL = leader _unitG;
_PosLand = position _UL;
_nothing = true;
_End = [((position leaderHQ) select 0) + (random 400) - 200,((position leaderHQ) select 1) + (random 400) - 200];
_rd = 200;
...
_posX = ((getPos leaderHQ) select 0) + _dXb + _dXc + (random 200) - 100;
_posY = ((getPos leaderHQ) select 1) + _dYb + _dYc + (random 200) - 100;

_MElevated = [_posX,_posY];
_MElev = (getPosATL (nearestObject [_posX,_posY,10])) select 2;

if (_unitG in RydHQ_FOG) then
	{
...
if (_isWater) exitwith {RydHQ_ReconAv = RydHQ_ReconAv + [(_unitG)];_unitG setVariable [("Busy" + (str _unitG)),false]};
...
if ((RydHQ_Debug) or (isPlayer (leader _unitG))) then
	{
	_i = [[_posX,_posY],_unitG,"markRecon","ColorRed","ICON","mil_dot","Rec A"," - NON-COMBAT RECON"] call RYD_Mark;
	};
...
_eClose1 = [[_posX,_posY],RydHQ_KnEnemiesG,400] call RYD_CloseEnemyB;
...
if not (isNull _NeN) then
	{
	_eClose2 = [_UL,RydHQ_KnEnemiesG,400] call RYD_CloseEnemyB;
	_tooC2 = _eClose2 select 0;
	_dstEM2 = _eClose2 select 1;
	_eClose3 = [leaderHQ,RydHQ_KnEnemiesG,400] call RYD_CloseEnemyB;
...
if (not (isNull _GDV) and (_GDV in (RydHQ_NCCargoG + RydHQ_AirG)) and (_NeNMode)) then {_LX1 = (position _UL) select 0;_LY1 = (position _UL) select 1;_halfway = true};
...
if ((isNull _AV) and (([_posX,_posY] distance _UL) > 1500) and not (isPlayer (leader _unitG))) then
	...
	_beh = "SAFE";
	if (_unitG in RydHQ_RAirG) then {_beh = "CARELESS"};
	_spd = "LIMITED";
	_TO = [0,0,0];
...
_tp = "MOVE";
//if (not (isNull _AV) and (_unitG in RydHQ_NCrewInfG)) then {_tp = "UNLOAD"};
_beh = "SAFE";
if (not (isNull _AV) and (_GDV in RydHQ_AirG) or (_unitG in RydHQ_RAirG)) then {_beh = "CARELESS"};
_spd = "NORMAL";
...
_cause = [_unitG,6,true,600,50,[RydHQ_AirG,RydHQ_KnEnemiesG],false] call RYD_Wait;
...
if not (_alive) exitwith {if ((RydHQ_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markAttack" + str (_unitG))}};
...
_cause = [_unitG,6,true,250,30,[RydHQ_AirG,RydHQ_KnEnemiesG],false] call RYD_Wait;
_alive = _cause select 1;

if not (_alive) exitwith {if ((RydHQ_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markRecon" + str (_unitG))}};
if ((count (waypoints _unitG)) >= 1) then {deleteWaypoint [_unitG, 1]};

if (_stage >= 4) then {RydHQ_ReconDone = true};
...
if (_unitG in RydHQ_FOG) then
	...
	_cause = [_unitG,6,true,150,120,[RydHQ_AirG,RydHQ_KnEnemiesG],false] call RYD_Wait;
	_timer = _cause select 0;
	_alive = _cause select 1;
	_enemy = _cause select 2
	};
...
if not (_alive) exitwith {if ((RydHQ_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markRecon" + str (_unitG))}};
if ((RydHQ_Debug) or (isPlayer (leader _unitG))) then {_i setMarkerColor "ColorBlue"};
...
while {(_nothing)} do
	...
	if (((not (isNull (_UL findNearestEnemy _UL)) or (_timer2 > 4)) and not (isNull _unitG) and not (_unitG in RydHQ_FOG)) or ((_timer2 > 40) and not (isNull _unitG))) then
		{
		if (_unitG in RydHQ_NCrewInfG) then {_pass orderGetIn true};
		sleep 15;
		_rd = 0;
		if (_unitG in RydHQ_AirG) then {_End = _PosLand;_rd = 0} else {_End = [((position leaderHQ) select 0) + (random 400) - 200,((position leaderHQ) select 1) + (random 400) - 200];_isWater = surfaceIsWater _End;if (_isWater) then {_End = [((position leaderHQ) select 0) + (random 40) - 20,((position leaderHQ) select 1) + (random 40) - 20]}};
		...
		if ((RydHQ_Debug) or (isPlayer (leader _unitG))) then {deleteMarker ("markRecon" + str (_unitG))};
		_nothing = false;
		...
		if (_alive) then
			{
			if (_unitG in (RydHQ_reconG + RydHQ_FOG + RydHQ_snipersG + RydHQ_InfG)) then {RydHQ_ReconAv = RydHQ_ReconAv + [_unitG];};
